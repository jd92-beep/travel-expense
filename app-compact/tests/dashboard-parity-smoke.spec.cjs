...
test('Dashboard travel-day widgets show countdown, receipt reminder, weather alert, and booking note', async ({ page }) => {
  ...
  const checklist = page.getByLabel('Departure checklist');
  await expect(checklist).toBeVisible();
  await expect(checklist).toContainText('Departure Checklist');
  await expect(checklist).toContainText('local · no API');
  await expect(checklist).toContainText('Weather kit');
  await expect(checklist).toContainText('雨具 / Umbrella');
  await expect(checklist).toContainText('Route kit');
  await expect(checklist).toContainText('IC / ticket ready');
  await expect(checklist).toContainText('Itinerary type');
  await expect(checklist).toContainText('Outdoor layer');
  await expect(checklist).toContainText('Temple Gate');
  await expect(checklist).toContainText('Booking check');
  await expect(checklist).toContainText('Ref ready');
  await expect(checklist).toContainText('KTX-5512');
  await expect(checklist).toContainText('Receipt habit');
  await expect(checklist).toContainText('Fix before moving');
  await checklist.getByRole('button', { name: /Weather/ }).click();
  await expect(page).toHaveURL(/#weather/);
  await page.getByLabel('主要分頁').getByRole('button', { name: '主頁', exact: true }).click();
