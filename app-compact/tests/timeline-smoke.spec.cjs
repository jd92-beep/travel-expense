...
test('Timeline shows deterministic per-day readiness score from itinerary, weather, booking, and cleanup signals', async ({ page }) => {
  const fixed = new Date('2026-05-09T09:30:00+09:00').valueOf();
  await page.addInitScript((fixedNow) => {
    window.__disable_supabase_configured = true;
    const RealDate = Date;
  ...
  const geometry = await readiness.evaluate((node) => {
    const box = node.getBoundingClientRect();
    const screen = document.querySelector('.timeline-screen')?.getBoundingClientRect();
    return {
      width: box.width,
