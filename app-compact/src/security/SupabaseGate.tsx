...
export function SupabaseGate({ auth, children }: SupabaseGateProps) {
  ...
  async function handleGoogleLogin() {
    ...
    try {
      ...
      // Native auth completes after the system browser returns through the app link.
    ...
    return (
      <main className="lock-screen compact-login-screen">
        <section className="lock-panel canva-lock-panel compact-login-panel compact-login-panel--loading" aria-label="Supabase reconnect">
          <div className="compact-login-brand-mark"><ShieldCheck size={24} /></div>
          <div>
            <p className="compact-login-kicker">Travel Expense</p>
            <h1>連接旅程雲端</h1>
            <p className="muted">正在安全確認你嘅登入 session。</p>
          </div>
        </section>
      </main>
    );
  ...
  return (
    <main className="lock-screen compact-login-screen">
      <section className="lock-panel compact-login-panel" aria-label="Travel Expense Supabase login">
        <figure className="compact-login-visual">
          <img src={travelAiAtlasImage} alt="旅途中整理收據、行程及雲端記錄" />
          <figcaption>每段旅程，都有一本清楚嘅帳。</figcaption>
        </figure>

        <div className="compact-login-content">
          <div className="compact-login-brand">
            <span className="compact-login-brand-mark" aria-hidden="true"><ShieldCheck size={20} /></span>
            <span>Travel Expense</span>
          </div>

          <div className="compact-login-head">
            <p className="compact-login-kicker">你嘅私人旅程帳簿</p>
            <h1>
          ...
          >
            ...
            {auth.error && <p className="lock-error compact-login-message" role="alert">{auth.error}</p>}
          ...
          <p className="compact-login-privacy">
            <ShieldCheck size={15} aria-hidden="true" />
            <span>每個帳號獨立保存資料，共用裝置亦可安全清除本機記錄。</span>
          </p>
        </div>
      </section>
    </main>
