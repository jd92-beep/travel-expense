...
assert.ok(syncing.idempotencyKey);
queue = settleChange(queue, syncing.id, { kind: 'syncing' }).queue;
queue = enqueueChange(queue, receipt('r1', { updatedAt: 20 }));
assert.equal(queue[0].idempotencyKey, syncing.idempotencyKey);
queue = enqueueChange(queue, receipt('r1', { supabaseId: undefined }));
...
assert.ok(Number(retryQueue[0].nextRetryAt) > Date.now());
...
let accessDeniedQueue = enqueueChange([], receipt('access-denied'));
accessDeniedQueue = settleChange(accessDeniedQueue, accessDeniedQueue[0].id, {
  kind: 'terminal-error',
...
const diagnostic = enqueueChange([], {
  ...receipt('diagnostic'),
  error: 'row-level security blocked',
});
assert.equal(diagnostic[0].error, 'row-level security blocked');

let terminalOverflow = enqueueChange([], receipt('terminal-overflow'));
terminalOverflow = settleChange(terminalOverflow, terminalOverflow[0].id, {
  kind: 'terminal-error',
  error: '40001 version conflict',
}).queue;
for (let index = 0; index < 501; index += 1) {
  terminalOverflow = enqueueChange(terminalOverflow, receipt(`overflow-${index}`));
}
assert.ok(terminalOverflow.some((item) => item.entityId === 'terminal-overflow'));
