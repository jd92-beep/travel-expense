...
export function Scan({
  ...
  batch: Array<Receipt & { selected?: boolean }>;
  setBatch: React.Dispatch<React.SetStateAction<Array<Receipt & { selected?: boolean }>>>;
}) {
  ...
  const handleImage = useCallback(async (file?: File, retry = false) => {
    if (!file) {
      if (mountedRef.current) setStatus('未收到圖片。相機無彈出時，請試相簿或手動記一筆。');
      return;
    }
    ...
    if (mountedRef.current) setStatus('讀取收據圖片…');
