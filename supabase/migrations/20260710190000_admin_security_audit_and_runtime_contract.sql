-- Keep denied admin traffic in the same append-only chain as successful
-- operations, and expose one fixed database runtime contract to service_role.
...
alter table private.admin_audit_events_v2
  alter column schema_version set default '20260710190000',
  alter column edge_version set default 'admin-kanban-v1';
...
declare
  ...
  v_schema_version constant text := '20260710190000';
...
declare
  ...
  v_session_hash text := case
    when p_session_hash ~ '^[0-9a-f]{64}$' then p_session_hash
    else encode(extensions.digest(coalesce(p_session_hash, 'unauthenticated'), 'sha256'), 'hex')
  end;
  v_method text := upper(coalesce(p_method, ''));
  v_route text := left(coalesce(nullif(p_route, ''), 'invalid'), 256);
  v_target_hash text;
begin
  if p_action not in ('admin_request_denied', 'admin_signature_rejected')
  ...
  return private.append_admin_audit_v2(
    ...
    null,
    null,
    jsonb_build_object('blocked', true, 'method', v_method, 'route', v_route),
    p_error_code,
    p_request_id,
    null,
    null
