-- Additive split editor storage. Applied via Supabase Management API; do not blind db push.
...
alter table public.receipts
  drop constraint if exists receipts_split_type_check;

alter table public.receipts
  add constraint receipts_split_type_check
  check (split_type is null or split_type in ('equal', 'shares', 'exact', 'percent', 'adjustment', 'itemized'));
...
begin
  ...
  if v_existing.id is null then
    ...
    values (
      ...
      nullif(btrim(p_receipt->>'split_type'), ''),
      case when jsonb_typeof(p_receipt->'splits') = 'array' then p_receipt->'splits' else null end,
      case when jsonb_typeof(p_receipt->'payers') = 'array' then p_receipt->'payers' else null end,
    ...
    set store = coalesce(nullif(btrim(p_receipt->>'store'), ''), store),
        ...
        split_type = nullif(btrim(p_receipt->>'split_type'), ''),
        splits = case when jsonb_typeof(p_receipt->'splits') = 'array' then p_receipt->'splits' else null end,
        payers = case when jsonb_typeof(p_receipt->'payers') = 'array' then p_receipt->'payers' else null end,
        deleted_at = null,
