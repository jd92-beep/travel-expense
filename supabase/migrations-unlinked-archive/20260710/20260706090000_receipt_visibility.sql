...
--
-- Rollback: drop policy receipts_select_trip_members and recreate the
-- 20260526071500 version; drop constraint receipts_visibility_check; drop column
-- visibility; re-apply the 20260613001000 version of upsert_shared_trip_receipt.
...
-- Recreate the shared-ledger RPC (it maps columns explicitly, so without this the
-- visibility field would be silently dropped for shared trips — exactly where privacy
-- matters). Two behaviour changes vs 20260613001000:
--   1. visibility is read from p_receipt (anything other than 'private' coerces to 'trip').
--   2. private receipts never enqueue a Notion mirror job and keep notion sync disabled —
--      the shared Notion database must not receive rows other members cannot see in-app.
