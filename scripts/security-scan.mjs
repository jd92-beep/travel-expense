...
const publicBuildPatterns = [
  { name: 'Direct Notion token prompt in public build', re: /Notion Integration Token|direct API|ntn_abc123/gi },
];
...
for (const rel of walk(repoRoot)) {
  ...
  if (rel.startsWith('_site/') || rel.startsWith('app-react/dist/')) {
    for (const { name, re } of publicBuildPatterns) {
      re.lastIndex = 0;
      let match;
      while ((match = re.exec(content))) {
        const line = content.slice(0, match.index).split(/\r?\n/).length;
