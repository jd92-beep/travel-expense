 ...
 *
 *     1. Kimi-8k           (Moonshot, primary fallback)
 *     2. Kimi-32k          (Moonshot, long-text fallback)
 ...
 *     6. Defer to next trigger cycle (up to 3 cycles)
 *
...
// ── CONFIG ─────────────────────────────────────────────────
// Kimi (Moonshot) API key — used for moonshot-v1-8k, moonshot-v1-32k
const KIMI_KEY = 'PASTE_KIMI_KEY_OR_LEAVE';
...
function extractBookingsWithFallback(emailText) {
  ...
  const errors = [];
  const hasKimi = KIMI_KEY && !KIMI_KEY.startsWith('PASTE_');
  ...
  const providers = [];
  if (hasKimi) {
    providers.push({ name: 'kimi-8k',  fn: () => _callKimiWithBackoff(prompt, 'moonshot-v1-8k') });
    providers.push({ name: 'kimi-32k', fn: () => _callKimiWithBackoff(prompt, 'moonshot-v1-32k') });
  }
  if (hasMinimax) {
...
function _callKimiWithBackoff(prompt, modelId) {
  const body = {
    model: modelId,
    messages: [
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    const resp = UrlFetchApp.fetch(KIMI_URL, {
      method: 'post',
    ...
    if (code === 429 || code === 503) {
      ...
      console.log('Kimi (' + modelId + ') ' + code + ', waiting ' + waitMs + 'ms…');
      Utilities.sleep(waitMs);
      continue;
    }
    throw new Error('Kimi ' + code + ': ' + resp.getContentText().slice(0, 200));
  }
  throw new Error('Kimi ' + modelId + ' 429 after 3 retries');
}
...
function pushToNotion(b, source, emailSubject, threadId, bookingIdx) {
  ...
  const pick = function() {
    for (var i = 0; i < arguments.length; i++) {
      if (dbSchema[arguments[i]]) return arguments[i];
    }
    return '';
  };
  ...
  const objectTypeKey = pick('Object Type', '物件類型');
  if (objectTypeKey) props[objectTypeKey] = { select: { name: 'receipt' } };
  const timeKey = pick('⏰ 時間', '時間');
  if (timeKey) props[timeKey] = { rich_text: [{ text: { content: (b.time || '').slice(0, 100) } }] };
  const addressKey = pick('🗺️ 地址', '地址');
  if (addressKey) props[addressKey] = { rich_text: [{ text: { content: (b.address || '').slice(0, 500) } }] };
  const bookingRefKey = pick('🎫 Booking Ref', 'Booking Ref');
  if (bookingRefKey) props[bookingRefKey] = { rich_text: [{ text: { content: (b.booking_ref || '').slice(0, 200) } }] };
  const currencyKey = pick('Currency', '幣種');
  ...
  const newPage = _notionFetchWithRetry('https://api.notion.com/v1/pages', 'post',
    JSON.stringify({ parent: { database_id: NOTION_DB }, properties: props }));
  if (newPage && newPage.id) {
    PropertiesService.getScriptProperties().setProperty('sid_' + sourceId, newPage.id);
  }
}
...
//   - Uses Apps Script PropertiesService to cache sourceId -> page_id mapping.
//   - Direct page GET allows checking 'archived' state even for deleted/archived rows
//     which are silently hidden by standard Notion database queries.
//   - This completely solves the "zombie resurrecting/ghost duplication" expense bug.
function _findNotionPageBySourceId(sourceId) {
  const scriptProps = PropertiesService.getScriptProperties();
  const cachedPageId = scriptProps.getProperty('sid_' + sourceId);

  if (cachedPageId) {
    try {
      // Direct GET is archive-aware and works even for archived pages!
      const page = _notionFetchWithRetry('https://api.notion.com/v1/pages/' + cachedPageId, 'get', null, 2);
      if (page) {
        return { id: page.id, archived: !!page.archived };
      }
    } catch (e) {
      // If 404/not_found, the page was permanently purged from trash — clear stale cache
      if (e.message.indexOf('404') >= 0 || e.message.indexOf('not_found') >= 0) {
        scriptProps.deleteProperty('sid_' + sourceId);
      } else {
        console.warn('[dedup] cached page GET failed, falling back:', e.message);
      }
    }
  }
  ...
  const dbSchema = _getDbSchema();
  try {
    var aliases = ['🔑 SourceID', 'SourceID'];
    for (var i = 0; i < aliases.length; i++) {
      var sidProp = aliases[i];
      if (!dbSchema[sidProp]) continue;
      // Notion DB queries hide archived pages, so this query will only return live entries
      ...
      if (live) {
        // Cache mapping for future cycles
        scriptProps.setProperty('sid_' + sourceId, live.id);
        return { id: live.id, archived: false };
      }
    }
    return null;
  } catch (e) {
...
function setup() {
  const providers = [];
  if (KIMI_KEY && !KIMI_KEY.startsWith('PASTE_'))       providers.push('Kimi-8k', 'Kimi-32k');
  ...
  if (!providers.length) throw new Error('⛔ Fill in at least one of KIMI_KEY / MINIMAX_TOKEN / OPENROUTER_KEY / GEMINI_KEYS');
