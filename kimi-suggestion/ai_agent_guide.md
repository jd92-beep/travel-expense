...
## ⚠️ Latest Problems (as of 2026-05-13)

### Problem 1: Mobile Chrome Still Flashes on Tab Switch
**Root cause**: WindmillTransition 150vmax + HyperframeBackground 4 layers (CSS-hides 2 on mobile but still mounts all 4) + NoiseTexture full-screen blend = GPU overload.
**Fix**: Reduce to 100vmax. Add React-conditional rendering in HyperframeBackground (mount only 2 layers on mobile). Add `content-visibility: auto` to glass cards.

### Problem 2: Camera/Gallery 2nd Attempt Fails
**Root cause**: `inputKey` increments AFTER `handleImage()` completes (in finally block). If user cancels picker, onChange never fires, inputKey never changes.
**Fix**: Move `setInputKey()` to triggerCamera/triggerGallery BEFORE `.click()`. Remove RAF wrapper.

### Problem 3: liquid-glass blur(32px) on Mobile
**Root cause**: No mobile media query override for `liquid-glass-enhanced` class.
**Fix**: Add `@media (max-width: 768px)` to reduce blur to 16px.

### Problem 4: Unused Imports in Dashboard.tsx
**Root cause**: `AnimatedCircularProgressBar` and `BorderBeam` imported but never used.
**Fix**: Remove unused imports.

## Resources

### pbakaus/impeccable (NOT phakerz — typo)
- GitHub: https://github.com/pbakaus/impeccable (27.8k stars)
- AI coding skill with 23 design commands
- Install: `cp -r dist/claude-code/.claude your-project/`
- Key commands: `/impeccable animate`, `/impeccable delight`, `/impeccable overdrive`

### transitions.dev
- URL: https://transitions.dev (plural)
- 12 copy-pasteable CSS transitions
- Good for reference

### React 19 <ViewTransition> (Recommended over WindmillTransition)
- 0 KB bundle cost (built into React)
- Wrap tab content with `<ViewTransition name="tab-content">`
- Replace WindmillTransition for better performance
...
| Fixed Bottom Tab Bar | 🔧 FloatingDock CSS fix (already installed) |
