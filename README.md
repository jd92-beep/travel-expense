...
旅程共享正在用「一個旅程、一套共享記帳簿」的方式建立。新的 React 和 Compact app 已經有共同資料結構、Welcome Guide 分享步驟、Settings `旅程共享` 管理卡，以及 Supabase migration/RLS 基礎。
...
- Editor 之後會用同一個共享 ledger 新增自己的記錄。
- Viewer 只應該看資料，不能新增或修改。
- Notion 會作為同一個 trip notebook 的後端副本，但正式雙寫仍要經 server-side mutation 和 retry queue，不應在瀏覽器直接暴露 Notion token。

注意：`supabase/migrations/20260612153000_trip_sharing_dual_backend.sql` 已於 2026-06-12 套用到 live Supabase project，migration 名稱是 `trip_sharing_dual_backend`。不要把 Supabase service role、Notion token 或 invite token 寫入前端或 GitHub。
