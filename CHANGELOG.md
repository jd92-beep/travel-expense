...
- **fix(db): key-presence UPDATE contract for shared receipts, live-applied.** Migration `20260926100000_upsert_receipt_key_presence_update.sql` was applied to live project `fbnnjoahvtdrnigevrtw` via the Management API (history divergence documented in HANDOVER still blocks `db push`; repair stays unapproved). `upsert_shared_trip_receipt` UPDATE assignments now change a column only when its key is present in the payload — a partial payload can no longer null out note/splits/etc., reset amount to 0, flip visibility back to 'trip', or overwrite record_date; present-but-empty still clears intentionally. Live `pg_get_functiondef` verification confirmed the guard. The remaining repo-pending migrations `20260916090000` (expense-comments visibility policy + lease index), `20260916110000` (shared-trip account-deletion ownership transfer) and `20260916130000` (service-role-only `admin_purge_user`, default-off) were reviewed and applied the same way; live verification confirmed the new comment policy respects receipt visibility, the lease index exists, the owner-transfer GUC guard is installed, and `delete_own_user_account` carries the hardened `search_path`.

- **chore(db): reconcile Supabase migration history; `db push` channel restored.** Live project `fbnnjoahvtdrnigevrtw` history now matches the repo 1:1 (69/69 versions, zero divergence): the seven repo migrations verified as live-equivalent (`20260724110000`, `20260806090000`, `20260824011000`, the `20260916` trio, `20260926100000`) were recorded via `migration repair --status applied`, and the three remote-only hotfix versions (`20260718170855`, `20260718170939`, `20260824033202` — the live twin of `20260824011000`) got committed placeholder files on `chore/supabase-history-reconcile`. `supabase db push --dry-run` now reports "Remote database is up to date". No live schema changed during reconciliation; HANDOVER's divergence caveat is closed.
...
- **perf(admin): console overhaul and production release train `1.3.5`–`1.4.1`.** Admin `1.4.1` is live (commit `9eb6627`, protected workflow `35483016157`, Vercel `dpl_CW3X2nrzhS5MDXN7LUncToEgcWwR`): framer-motion removed, lazy shell with optimistic login and font preload, augmented-ui purged from CSS, UX pass (danger purge styling, ConfirmDialog, toasts, pagination persistence, filter unification), a fresh-browser login CSRF fix (`1.3.9`), Edge provenance SHA baked at deploy time, readiness 503 reason codes, and a smoke-race fix. Post-bootstrap fresh-browser login passed live on `1.3.9` and was re-verified on `1.4.1`.
- **feat(admin/db): gated purge path and shared-trip account-deletion transfer.** A reviewed `admin_purge_user` path exists but stays default-off with Notion auto-archive on purge; live `delete_own_user_account()` now transfers shared-trip ownership, demotes the old owner, deletes private receipts, and leaves trip-visible receipts with the successor, proven by a live transaction/rollback SQL smoke.
- **fix(broker): restore the theme catalog and close photo/rate-limit gaps.** Broker redeployed as `2026.09.15.1` with the theme catalog restored; Compact+broker no longer clobber or re-upload photos, and the rate-limit Origin bypass is closed.

## 2026-08-24

- **security(frontend/backend): retire the unsafe legacy root and close credential, tenant and Storage gaps.** The Pages root is now a stateless CSP-protected redirect to Compact; React `0.2.7` and Compact `0.17.1` no longer ship an offline password verifier or unlock when Broker verification fails. The forward-only Supabase migration now runs live as `20260824033202_harden_receipt_ownership_and_photo_storage`, restricting Notion enqueue and child-row reparenting to the receipt owner, making receipt photos private, and enforcing a 6 MB image MIME allowlist. Live metadata was verified; an ordinary authenticated JWT photo smoke remains follow-up evidence.
- **security(broker): bound paid-provider traffic and make production limits atomic.** Broker `2026.08.24.1` validates AI kind/model/prompt/image inputs, caps output tokens, quotas AI/weather/provider probes, and uses a SQLite-backed Durable Object for atomic unlock/admin and daily quota counters. Cloudflare version `d86be18a-27a7-48dd-97a4-c9f47dd31733` is live; check, self-test, dry-run, live health/CORS/no-session smoke and a redacted unlock/status path passed.
...
- **fix/deploy(admin/web): package Admin BFF dependencies and complete every web deployment.** Admin `1.3.4` replaces an archive-escaping provider-catalog import with the contract-verified embedded catalog and adds a release-manifest regression gate. Protected workflow `32687928249` passed and promoted exact SHA `d92edfd3694e12e92651a8b43401624bb75f4c41` as `dpl_67SXrHRZoxKP1C7jkssnEThxSWDL`; live health is ready and the unauthenticated session route returns `401`. Compact `0.17.1` and React `0.2.7` are live on Vercel, Netlify and GitHub Pages.
...
- **v0.12.8 / versionCode 1208 — 3-agent review fix pass (11 bugs).** Found via parallel money/UI/native review of the v0.12.7 build; all verified on the emulator + smokes.
  - **[HIGH] Recurring rules spawned duplicate expenses forever.** `nextRunDate`/`today` formatted local dates with `toISOString()` (UTC), so in HKT a daily rule's `nextRun` never advanced and re-spawned the same day on every launch. Now uses local YMD + `todayYmd()`, with a bounded catch-up loop, `tripId`, and `updatedAt`. (`domain.ts`)
  - **[HIGH] Decimal point couldn't be typed in split / payer / amount inputs.** A `value={Number(...)}` controlled input stripped a trailing "." on re-render, so `33.34`/`1.5` were impossible (split editor is daily-use). New shared `NumberTextInput` keeps the raw typed string; applied to split, payer, total, original-amount, and batch-confirm fields. (`ui.tsx`, `ReceiptEditor.tsx`, `Scan.tsx`)
  - **[MED] Recurring receipts never reached the cloud** — they used `updateState` (settings-only queue); now routed through `upsertReceipt` so they enqueue. (`App.tsx`)
  - **[MED] Stale stored session** rendered as "cloud sync active" against a dead token; `storedSupabaseSession` now rejects expired `expires_at`. (`App.tsx`)
  - **[MED] Magic-link / email-confirm could strand native users** — `handleNativeAuthRedirectUrl` now handles `?token_hash=&type=` via `verifyOtp`, not just code/implicit. (`supabase.ts`)
  - **[MED] Cross-currency exact/multi-payer splits mis-settled** (split amounts in receipt currency vs trip-currency total) — now redistributes the converted total by each person's share via `computeShares('shares')`; identity when currencies match (JPY trips unaffected). Locked with a unit test. (`domain.ts`)
  - **[LOW] Welcome-guide hardware-back** silently provisioned a placeholder trip → excluded from back-dismiss. **Onboarding tip** dismiss now updates state (re-renders). **Photo-retry counter / photoUrl** preserved on concurrent-edit-during-sync. **Dead `window.open` photo fallback** removed. (`App.tsx`, `Dashboard.tsx`, `useSyncEngine.ts`)
  - Verified: `typecheck`, unit (split-engine + new cross-currency, notion-split-meta, sync-backoff), smokes (split-editor, settle-up, split-payer, scan, history 8/8, welcome-guide), `android:qa` (passed, App Links verified, no crash), and on-device decimal-input check (`1234.` preserved).
...
## GEMINI.md 架構升級紀錄（2026-06 ~ 2026-07）

> 由 `gemini.md` 搬過嚟嘅歷史 snapshot（原本係 §0–§12）。當時嘅事實，唔係現行 contract；
> 現行規則見 `AGENTS.md`，現況見 `HANDOVER.md`。
