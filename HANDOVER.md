...
## Last Worked On
- **Date**: 2026-07-01 HKT
- **Focus**: Comprehensive bug hunt — 3 parallel review agents (self-audit / React state / money engine) + a dynamic runtime-error walk + full smoke suite, all findings verified against code before fixing
- **Agent**: Oscar (Claude Code) on the isolated android worktree
- **App version**: Compact/Android `0.12.17` (versionCode `1217`); React unchanged
- **Latest pushed branch commit**: pending this session's push — preceded by `df7046c` (v0.12.16, hardware-back overlay closing + scan crash-recovery), `762f470` (v0.12.15, WebP asset slim-down + native UX/security council fixes), `429a982` (v0.12.14, reconnect sync hardening by another agent), `9ce635e` (v0.12.13, dropped unused 39MB video).
