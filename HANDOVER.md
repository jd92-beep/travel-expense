...
- Latest compact app-code proof commit: `8fadfcb` (`Add compact public diagnostics preview`).
...
- 2026-06-08 compact P4 progress:
  ...
  - Current compact checklist focus: retry P10-04 Vercel deployment after the Vercel daily deploy quota resets, then run `npm run smoke:deploy-live`. P0-05 remains `LIVE` because full provider-vault proof is still blocked by Kimi quota, required Google/Gemma model availability, Mimo authenticated provider config, and Wrangler account visibility. Keep broker-vault output redacted and secrets out of git.
