...
- Current authenticated P0-05 result: `npm run smoke:broker-vault` exits `2` with clean redacted JSON. Passed portions: broker session auth, `/credentials/status`, Notion/database permission via `/credentials/test-all`, WeatherAPI forecast with hourly and feels-like data, and Google diagnostic route/key using `gemini-2.5-flash`. Remaining blockers: Kimi account quota, required Google/Gemma model `gemma-4-31b` unavailable on the live Gemini API, and Mimo authenticated provider backend/config returning `404`.
