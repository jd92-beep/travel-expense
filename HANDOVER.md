...
- Latest compact app-code proof commit: `a1d2fe2` (`Simplify compact home dashboard`).
- Latest compact production deployment: Vercel deployment `dpl_GdfqimEjDaiC6mYCxB6v4c1rAKHX`, live URL `https://travel-expense-compact.vercel.app/`, HTTP 200, title `旅費 Compact`, asset hash `1776760a3f776a73`, HTML hash `d0b7c079205d26d3`, and alias/deployment content match for commit `a1d2fe2`.
...
- 2026-06-09 compact Home simplification pass: Dashboard/Home no longer renders `Travel-day control`, `Itinerary Receipt Match`, `Trip Snapshot`, `Departure Checklist`, `Day-end Closeout`, or the bottom `旅程提醒` accordion. The Dashboard-only derived data/imports/state for those cards were removed, while shared `travelDay` helpers remain for Timeline and History. Verification passed: GitNexus impact on `Dashboard` was LOW before editing; GitNexus detect-changes reported HIGH because `Dashboard` appears in 6 indexed flows; `npm run typecheck`; `node scripts/run-with-dev-server.mjs -- npm run smoke:dashboard` (5 passed); `git diff --check`; `npm run smoke:production-gate` (224.7s); `npm run smoke:deploy-live` for deployment `dpl_GdfqimEjDaiC6mYCxB6v4c1rAKHX`.
