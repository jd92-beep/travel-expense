...
## Last Worked On
- **Date**: 2026-06-21 HKT
- **Focus**: Two PUA bug-fix passes (v0.12.10 + v0.12.11) — static native audit + dynamic mutation-orphan audit
- **Agent**: Oscar (Claude Code) on the isolated android worktree
- **App version**: Compact/Android `0.12.11` (versionCode `1211`); React unchanged
- **Latest pushed branch commit**: `eacaa2e` (`fix(data): clean orphan refs on person removal + tombstone trip-cascade deletes v0.12.11`), pushed and in sync with `origin/codex/android-compact-shell`. Preceded by `c06b520` (v0.12.10, export filename fix) and `1a08958` (v0.12.9 docs sync).
- **Current branch state**: `codex/android-compact-shell` tracking `origin/codex/android-compact-shell`, clean working tree, local == upstream. All roadmap phases remain complete; latest commits are pure bug-fix passes on top of the finished roadmap. **Do not merge to `main`** (friend uses the live compact web served from main).
- **What v0.12.10 + v0.12.11 fixed** (full list in `CHANGELOG.md`):
  - **v0.12.10 — native export filename sanitization:** `Filesystem.writeFile` treats `path` as a filesystem path — trip names with `/`, `:`, `*` etc. caused silent write failure. Fixed at the `saveFile` choke point in `domain.ts` (strips `[/\\:*?"<>|\x00-\x1f]+`, leading dots; applies to writeFile path, getUri, Share title, blob download attr). All callers safe.
  - **v0.12.11 — data-integrity (mutation-orphan audit):** [BUG-2] `removePerson` only remapped top-level `personId`; now also clears `splits[]`/`payers[]` (resets `splitType`) and filters `lineItems[].assignedTo`. [BUG-3] `handleDeleteManagedTrip` cascade-deleted receipts but never wrote tombstones → autoSync pull could resurrect orphaned rows; now mirrors `deleteReceipt` pattern (`notionDeletedSourceIds` + `notionDeletedIds`, cap 5000).
- **Latest verification evidence**: v0.12.11 passed `npm run typecheck` (clean, 2375 modules), full smoke suite with live dev server (dashboard 8/8, history 8/8, split-editor 1/1, timeline 8/8, stats 1/1, welcome-guide 1/1 — all green), and `explore-errors.mjs` runtime walker (7 tabs + ReceiptEditor with stress seed data — zero runtime exceptions). Signed release APK (66 MB) built and delivered to Boss.
- **Current known verification blockers / pending**: none. Real-device Google/magic-link login still needs a human account/device round-trip. Build Versioning Rule "currently at": v0.12.11 / versionCode 1211.
...
- **Bug-review fix passes v0.12.7 → v0.12.11 (post-roadmap hardening):** v0.12.7 Codex polish (configured-login safe-area + native picker console cleanup, QA harness flakes). v0.12.8 fixed 11 bugs from a 3-agent review (recurring UTC dupes, decimal-input stripping, recurring-not-syncing, stale session, magic-link stranding, cross-currency mis-split). v0.12.9 fixed 10 findings from an adversarial verification workflow (photo-sync data-loss, PKCE re-consumption, double-rounding, auth-error mis-parking, UTC off-by-one sweep, tombstone caps). v0.12.10 fixed native export filename silent failure. v0.12.11 fixed mutation-orphan bugs (person removal + trip cascade tombstones). All on the finished roadmap — no new features. See `CHANGELOG.md`.
