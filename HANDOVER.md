...
- Latest pushed compact app-code commit: `e94db31` (`Queue compact trip settings sync`).
- Latest compact production deployment: Vercel deployment `dpl_2rvr48g1yNbfx6KcveppjbEbrgFG`, live URL `https://travel-expense-compact.vercel.app/`, HTTP 200, title `旅費 Compact`, asset hash `a0a8fde5209a9028`, HTML hash `31b761c4d8028adf`, and alias/deployment content match after docs HEAD `6b402d0` / app-code commit `e94db31`.
- 2026-06-11 compact Dashboard Broker AI Assistant removal: Removed the Broker AI Assistant panel card from the Home tab based on Boss's request to declutter the home page. This includes removing the input field, state, handler logic, and associated `.dashboard-broker-assistant` styling from `compact.css`, as well as cleaning up Playwright assertions from `tests/dashboard-parity-smoke.spec.cjs`. Verification passed via `app-compact npm run typecheck`, production build, and `smoke:dashboard` test suite.
- 2026-06-11 compact Jeju trip sync root-cause fix: Settings Trip Update `applyTripDraft()` was correctly updating the local active trip and queuing a `trip` sync item, but it did not queue the app-level `settings` sync item that carries `activeTripId`, active trip currency/date range, and settings freshness. That meant a Jeju trip could appear updated locally while Supabase/Notion profile state still pointed at the previous active-trip settings, especially after a pull or on another device. `applyTripDraft()` now queues both `trip:<activeTripId>` and `settings:app-settings`, stamps `settingsUpdatedAt`, and de-duplicates stale trip/settings queue entries before appending the fresh pair. Regression coverage now asserts the Jeju Trip Update confirmation writes the 8-day itinerary and queues both sync items. Verification passed: GitNexus impact LOW for compact `applyTripDraft()` and `useSyncEngine`; targeted Settings Trip Update smoke first failed on missing `settings` queue then passed after the fix; `app-compact npm run typecheck`; `app-compact npm run build`; `node scripts/run-with-dev-server.mjs -- npm run smoke:final-nav` (`8 passed`). Direct `npm run smoke:final-nav` without the dev-server wrapper failed only with `ERR_CONNECTION_REFUSED`; `npm run smoke:supabase-notion-mirror` skipped its provider-gated cases in the local environment.
- 2026-06-11 compact Trip Update canonical-first architecture pass: `parseTripParagraph()` now uses a two-stage LLM workflow for Trip Update. Stage 1 sends the user's raw pasted itinerary to the selected/fallback provider JSON route only to produce the model's own `organizedItinerary`; Stage 2 sends only that canonical itinerary back to the model and extracts the app `trip.itinerary`, metadata, and `extractionReport` from it. This deliberately prevents the old concept of scraping final app fields directly from arbitrary user input formats. `TripDraft` now has optional `organizedItinerary`, Settings shows `AI 重整行程` in the preview/confirmation modal, and `app-compact/scripts/trip-update-live-smoke.mjs` now proves organize/extract as two separate broker calls. The compact frontend Trip Update path no longer depends on the old one-shot `/trip/intelligence` route, although the Worker route still exists for compatibility/self-tests. GitNexus impact was HIGH for compact `parseTripParagraph()` because Settings, Dashboard trip creation, and Welcome Guide call it; `TripDraft` fanout was CRITICAL, mitigated by adding an optional field only. Verification passed: `app-compact npm run typecheck`; Settings Trip Update smoke (`3 passed`); AI routing smoke (`4 passed, 1 skipped`); `app-compact npm run build`; `app-compact npm run security:scan`; mobile layout smoke (`1 passed` through dev-server wrapper); live broker Trip Update smoke using `gemini-3.1-flash-lite` (`organizeStatus=200`, `extractStatus=200`, `8820ms`, 8 days, 30 spots, `organizedItinerary` present, redacted/no secrets).
- 2026-06-11 Trip Update AI pasted-itinerary extraction fix: `tripIntelligencePromptContract()` now tells LLMs to handle Markdown headings, pipe tables, pasted `<br>` separators, Chinese dates, English dates, and plain timetables, and to treat pipe-table rows as real itinerary spots. `localTripDraftFromParagraph()` now normalizes `<br>`, supports `### Day`, Chinese `6月13日`, English `Jun 13, 2026`, `住宿：` lines, pipe-table rows, and AM/PM plain rows, so Settings can still open the confirmation modal when providers fail or return unusable output. GitNexus impact was HIGH for `parseTripParagraph`, `localTripDraftFromParagraph`, and `tripIntelligencePromptContract` because Settings, Dashboard trip creation, and Welcome Guide all use the parser. Verification passed: `app-compact npm run typecheck`; focused local-parser smoke (`2 passed`); Settings Trip Update smoke (`3 passed`, including the new markdown-table regression); AI routing smoke (`4 passed, 1 skipped`); `app-compact npm run build`; `app-compact npm run security:scan`; live broker LLM table proof using `gemini-3.1-flash-lite` (`6660ms`, 8 days, 17 spots, 3 lodging entries, redacted output).
- 2026-06-11 Mimo v2.5 Pro model-option pass: both Compact and React `AI_MODELS` now include `mimo/mimo-v2.5-pro` labelled `Mimo v2.5 Pro`, so Settings model routing can choose it for scan/voice/email/trip-update slots. No new frontend secret or Worker credential was added; the existing `/mimo/json` broker route forwards the requested model and uses the same vaulted Mimo base URL/API key as `mimo-v2.5`. GitNexus impact for both `AI_MODELS` constants was LOW with no affected recorded execution flows. Verification passed: `app-compact npm run typecheck && npm run build && npm run security:scan`; `app-react npm run typecheck && npm run build && npm run security:scan`; Compact Settings smoke (`1 passed`); Compact selected-Pro AI routing smoke (`1 passed`, proving provider `mimo`, model `mimo-v2.5-pro`); React Settings smoke (`1 passed`); live broker Pro smoke (`mimo-v2.5-pro`, HTTP 200, 42810ms, 8 days, 32 spots). Compact production deployment passed as Vercel `dpl_6K3CfmwH5C54dN298bYTvud6vsTi`, live asset hash `8957d9135539e565`, and a direct live bundle scan found `Mimo v2.5 Pro` in `ui-hJ4nDzUD.js`. Note that Pro works but remains slower than optimized `mimo-v2.5` and the Google fast trip fallback.
- 2026-06-11 live Mimo v2.5 speed fix: the Credential Broker Mimo JSON route was still sending plain OpenAI-compatible chat requests, so Xiaomi MiMo's reasoning path stayed enabled and itinerary extraction could take 40s+. `workers/credential-broker/src/index.js` now sends `thinking: { type: "disabled" }`, `stream: false`, and capped `max_tokens` on both `/mimo/json` and credential-test calls. Worker self-test now asserts those exact outgoing Mimo payload fields, and `app-compact/scripts/trip-update-live-smoke.mjs` can target `/mimo/json` by setting `COMPACT_TRIP_UPDATE_LIVE_PROVIDER=mimo`. GitNexus impact was LOW for `mimoJson`, `mimoChatCompletion`, and `mimoJsonWithCredential`, affecting only the Mimo broker route/provider-test path through `handleRequest`. Verification passed: `workers/credential-broker npm run check`; `workers/credential-broker npm run self-test`; `workers/credential-broker npm run preflight:deploy`; Cloudflare Worker deploy version `fb3a389b-dd50-425f-88d0-a228098a95eb`; `app-compact npm run typecheck`; `app-compact npm run security:scan`; `app-compact npm run build`; `app-compact npm run smoke:broker-vault`; Mimo live trip smoke (`mimo-v2.5`, 22376ms, 8 days, 32 spots); Google live trip smoke (`gemini-3.1-flash-lite`, 6443ms, 8 days, 32 spots); Settings Trip Update smoke (`2 passed`); AI routing smoke (`4 passed, 1 skipped`). Prior baseline before the Worker fix was `mimo-v2.5` taking about 44.7s even on a 2-day extraction, so the new direct Mimo path is materially faster but still slower than the Google trip fallback.
- 2026-06-11 compact Trip Update AI no-response root-cause and fast-path fix: live broker/provider proof showed all providers connected, but real itinerary extraction was slow on selected Mimo (`mimo-v2.5` took 38.3s for a 2-day extraction), while `google/gemini-3.1-flash-lite` extracted an 8-day Jeju itinerary in 5.3s with 8 days and 32 spots. `parseTripParagraph()` now keeps the selected model as the first attempt, but applies trip-attempt timeouts, skips retrying the same slow Kimi route after a structured timeout, and moves `gemini-3.1-flash-lite` plus `gemini-2.5-flash` into the trip fallback ladder before slow Mimo/Gemma fallbacks. For pasted day-by-day itineraries, the local parser prepares a confirmation-ready draft immediately and returns it if provider analysis exceeds the fast response window, so the Settings card does not sit in a silent loading state. Added `npm run smoke:trip-update-live`, which uses the ignored broker-vault session and redacted output to prove live LLM trip extraction speed without printing secrets. Verification passed: `app-compact npm run typecheck`; `app-compact npm run build`; `app-compact npm run security:scan`; `app-compact npm run smoke:broker-vault`; `app-compact npm run smoke:trip-update-live` (`gemini-3.1-flash-lite`, 5312ms, 8 days, 32 spots); Settings Trip Update smoke (`2 passed`); AI routing smoke (`4 passed, 1 skipped`); Dashboard smoke (`9 passed`); `git diff --check`. GitNexus impact for compact `parseTripParagraph()` was HIGH because it feeds Settings, Dashboard new-trip creation, and Welcome Guide.
- 2026-06-11 compact Jeju/Korea currency, model-routing, Trip Update, and Weather recovery pass: Home budget display now toggles between HKD and the active trip currency instead of hardcoded JPY, so Korea/Jeju trips show KRW even if old local state stored `displayCurrency: "JPY"`. Record manual entry defaults `原貨幣` from the itinerary day or active trip currency, and shared currency conversion now supports KRW plus common travel currencies without falling back to the JPY rate. Dashboard Broker AI Assistant now shows and calls the selected trip-update model as primary instead of hardcoding Kimi. Trip Update AI now walks the model ladder on provider failures and has a local itinerary parser fallback for long pasted day-by-day itineraries; the fallback opens the confirmation modal with extracted Jeju days, lodging, spots, and warnings rather than loading silently. Weather fallback now falls through from short WeatherAPI ranges to Open-Meteo when the target travel date is outside the broker forecast window, preventing empty weather cards for future itinerary dates. GitNexus impact was CRITICAL because the changed symbols touch trip creation, receipt stamping, Dashboard, Settings, Weather, and shared conversion flows; verification passed with `app-compact npm run typecheck`, `app-compact npm run build`, `app-compact npm run security:scan`, `git diff --check`, targeted Dashboard smoke (`9 passed`), targeted Settings Trip Update smoke (`2 passed`), AI routing smoke (`3 passed, 1 skipped`), and `app-compact npm run smoke:deploy-live` after the production deploy.
- 2026-06-11 shared React/Compact itinerary normalization pass: both `app-compact/src/domain/trip/normalize.ts` and `app-react/src/domain/trip/normalize.ts` now normalize itinerary dates without relying on timezone-sensitive `new Date()` for common travel inputs. Full dates such as `2026/6/13`, `2026.6.13`, and `2026年6月13日` become `2026-06-13`; month/day-only values such as `6/13` or `6月13日` infer the year from another itinerary day or the trip id before falling back to the current year. This preserves stable `dayId`/`spotId` generation for Timeline, Weather, Settings, Stats, Supabase, and Notion sync. GitNexus impact was CRITICAL for `normalizeItinerary()` on both Compact and React because it feeds trip hydration and sync flows; direct verification passed with `app-compact npm run typecheck` and `app-react npm run typecheck`, and broader smoke/build checks should be run before claiming live production proof.
- 2026-06-11 compact Settings and trip-entry stack from the previous agent: new-trip and trip-update textareas are taller with richer itinerary/booking placeholders; currency choices include more Asian/Oceania currencies; Trip Update AI prompt/review now asks for end times, lodging check-in/check-out, coordinates, spot type, and day highlights; the confirmation modal shows clearer hotel/restaurant/scenery counts, coordinate pins, and lodging details; weather guardrails coerce string `lat/lon`, handle missing spot arrays, and suppress duplicate city text; Settings hides developer-only diagnostics/stress/deploy tools behind the developer panel, keeps AI Models visible, adds `View / Edit Itinerary` to Trip Manager, simplifies Data Management, and defaults new compact users to one traveller.
- 2026-06-11 compact Settings/smoke cleanup pending commit in this handoff: `Email / Shortcut` is visible again in normal Settings while heavier diagnostics stay developer-gated. Compact smoke fixtures were updated for three Settings quick controls, deterministic Playwright clocking, dev-panel-only test cards, the current weather feels-like value, revised expense/settlement expectations, and the official-first JMA path in the multi-city Weather smoke. Verification passed: `app-compact npm run typecheck`; `app-react npm run typecheck`; `app-compact npm run security:scan`; `app-compact npm run build`; `app-react npm run build`; combined changed compact smoke suite with 40 Playwright tests passed across a11y, Dashboard, History, Scan, Settings, Weather, and Welcome Guide.
- 2026-06-10 compact Settings Trip Update AI confirmation fix: pasting a long itinerary and pressing `用已選模型分析` now opens a real `確認 AI 行程更新` modal immediately after model extraction, instead of relying only on an inline preview that could feel like no response. The modal shows the selected primary model, source quality, trip name/date/destination, extraction counts, hotels, restaurants, missing fields, assumptions, warnings, and all extracted itinerary days with lodging and time-ordered spots. `返回修改文字` closes the modal so the pasted text can be edited/re-analyzed, and `確認並更新行程` is the only local apply action. Verification passed: GitNexus impact was LOW for compact `Settings` and `applyTripDraft`; `parseTripParagraph` impact remains HIGH because it is shared with Dashboard, so parser behavior was not changed. `app-compact npm run typecheck`; `node scripts/run-with-dev-server.mjs -- npm run smoke:settings` (9 passed, including an 8-day Jeju confirmation/apply regression); `node scripts/run-with-dev-server.mjs -- npm run smoke:ai-routing` (3 passed, 1 skipped); `node scripts/run-with-dev-server.mjs -- npm run smoke:mobile-layout`; `npm run smoke:production-gate` (85.5s); Vercel production deploy; `npm run smoke:deploy-live` for deployment `dpl_7fsr8nN2MzWwL1e2Faz6yEoqbZaQ`.
- 2026-06-10 compact Mimo/Gemma routing fix: compact now uses exact Google Gemini API model id `google/gemma-4-31b-it` for the displayed `Google Gemma 4 31B` option, migrates stored stale `gemma-4-31b` backup settings to `gemma-4-31b-it`, and updates AI routing/shared-contract proof to expect the exact id. Credential Broker source now defaults Google to `gemma-4-31b-it`, parses the first balanced JSON object/array from provider responses so Gemma output with trailing text no longer fails JSON extraction, and routes Mimo calls with Xiaomi's documented `api-key` header plus a pay-as-you-go base fallback after Token Plan 404s. Verification passed: official Google Models API docs checked for exact `models.list` naming, Xiaomi MiMo docs checked for Token Plan base URLs and `api-key` header; GitNexus impacts were LOW for `mimoJson`, `mimoJsonWithCredential`, `googleJson`, MEDIUM for Worker `extractJson`, and CRITICAL for compact `normalizeAiModelSettings` because settings normalization touches app state; `workers/credential-broker npm run check && npm run self-test`; `app-compact npm run typecheck`; `node scripts/run-with-dev-server.mjs -- npm run smoke:ai-routing` (3 passed, 1 skipped); `npm run smoke:shared-contract`; `npm run smoke:production-gate` (79.7s); `git diff --check`; GitNexus detect-changes MEDIUM; Vercel production deploy; `npm run smoke:deploy-live`. Cloudflare deploy auth was later fixed via Wrangler OAuth, and the Worker source fix is now deployed live as version `7524c7dd-428a-4c11-bd75-bc6ebffeb20e`.
- 2026-06-10 Cloudflare deploy permission recovery: local Wrangler OAuth was refreshed after backing up the old Wrangler config, `npx wrangler whoami` and `workers/credential-broker npm run preflight:deploy` now pass, and `npx wrangler deploy` succeeded for `travel-expense-credential-broker` at `https://travel-expense-credential-broker.ftjdfr.workers.dev` with version `7524c7dd-428a-4c11-bd75-bc6ebffeb20e`. The Mimo Token Plan credential was rotated into the Credential Broker vault with redacted output only; no API key, OAuth token, broker session, or account secret was committed or printed. Direct broker proof passed: authenticated `/mimo/json` returned HTTP 200 with provider/model `mimo-v2.5`. `app-compact npm run smoke:broker-vault` now reaches all provider paths and passes Notion status/test-all, WeatherAPI forecast, Google JSON, Google diagnostic, and Mimo JSON; it still exits `2` only because Kimi is account-limited for the current billing cycle, which is expected and must not be bypassed with fallback calls.
- Previous compact app-code proof commit: `a2e9b72` (`Harden compact itinerary extraction`).
- 2026-06-10 compact itinerary extraction hardening: trip-update/new-trip model guidance now requires an app-wide `extractionReport`, expanded `countryCode` values (`JP`, `KR`, `TW`, `GB`, `EU`, `HK`, `CN`, `SG`, `TH`, `MY`, `VN`, `PH`, `AU`, `NZ`, `US`, `GLOBAL`), source quality, assumptions, missing critical fields, confidence, source text, booking references, and richer city/country/timezone/address/map metadata. `normalizeTripDraft()` no longer treats a missing itinerary as successful by copying the current itinerary, so the model ladder continues instead of silently reusing stale data. Selected Kimi trip updates now try the structured `/trip/intelligence` route first, then the same Kimi `/kimi/json` route before moving to Mimo/Google fallbacks. `validateItinerary()` now preserves spot and lodging metadata used by Timeline, Weather, and backend sync. Settings trip-update preview now shows source quality, transport count, missing critical fields, and model assumptions before applying the update. Verification passed: GitNexus impacts on `parseTripParagraph`, `normalizeTripDraft`, and `tripIntelligencePromptContract` HIGH plus `validateItinerary` LOW; `npm run typecheck`; `npm run smoke:settings` (8 passed); `npm run smoke:ai-routing` (3 passed, 1 skipped); `npm run smoke:trip-intelligence` (1 passed); `npm run smoke:dashboard` (8 passed); `npm run smoke:production-gate` (75.2s); `git diff --check`; manual Vercel production deploy; and `npm run smoke:deploy-live` for deployment `dpl_9y7VSDw6f7n1i9FjGixc7BWGFZ1U`.
- Previous compact app-code proof commit: `9e93aab` (`Use selected trip update model`).
- 2026-06-10 compact Settings trip-update model-routing fix: Settings no longer labels the trip-update card as Kimi-only. The card is now `Trip Update AI` / `AI 行程更新`, shows the currently selected `Trip update model`, and the analyze button says `用已選模型分析`. `parseTripParagraph()` now uses `state.tripUpdateModel` as the first trip-update attempt; a selected Google/Mimo model goes directly through its provider JSON route instead of hard-calling Kimi `/trip/intelligence`, while a selected Kimi model still uses the structured trip-intelligence route first. Empty/non-useful itinerary results continue down the trip model ladder, while quota/429/rate-limit errors remain hard stops. The preview now reports extracted day count, scenery spot count, hotel count, restaurant/food count, important-detail count, hotel/food/detail names, and compact per-day spot rows before anything is applied. Verification passed: GitNexus impact on `parseTripParagraph` and `Settings` LOW before editing; GitNexus index rebuilt successfully after stale status; GitNexus detect-changes HIGH because `Settings` participates in multiple flows, mitigated by `npm run typecheck`, manual single-dev-server `npm run smoke:settings` (8 passed), manual single-dev-server `npm run smoke:ai-routing` (2 passed, 1 skipped), `npm run smoke:production-gate` (86.8s), `git diff --check`, manual Vercel production deploy, and `npm run smoke:deploy-live` for deployment `dpl_3eB3LEyVJ9HbGKtNw6nCMjwh4ABn`.
- Previous compact app-code proof commit: `fd9b3a1` (`Add trip wizard LLM fallback ladder`).
- 2026-06-10 compact trip AI fallback ladder pass: new-trip wizard creation no longer goes straight from the primary/default trip model to curated scenery fallback. `parseTripParagraph()` now treats an empty primary `/trip/intelligence` result as unusable and then uses the existing model ladder via `/kimi/json`, selected trip model, Mimo, Google Gemma/Gemini fallbacks, and only returns fallback draft after all non-quota attempts fail. Dashboard keeps the wizard open on quota/429/rate-limit errors and shows a no-bypass error instead of silently creating default scenery spots. Curated Jeju/scenery fallback still exists, but only after the LLM ladder fails without quota/rate-limit. Verification passed: GitNexus impact on compact `parseTripParagraph` LOW; GitNexus detect-changes LOW; `npm run typecheck`; `node scripts/run-with-dev-server.mjs -- npm run smoke:dashboard` (8 passed, including LLM fallback before scenery fallback and quota hard-stop); `node scripts/run-with-dev-server.mjs -- npm run smoke:ai-routing` (2 passed, 1 skipped); `npm run smoke:production-gate` (99.4s); `git diff --check`; manual Vercel production deploy; `npm run smoke:deploy-live` for deployment `dpl_8ypkt9eQeAcbgYceSoo242uSknBS`.
- Previous compact app-code proof commit: `d325709` (`Fix compact trip wizard itinerary creation`).
- 2026-06-10 compact Home new-trip AI itinerary creation fix: the Home dropdown `建立新旅程` wizard no longer discards Step 4 `旅程詳情`. On create, Dashboard now builds a trip-analysis paragraph from the wizard fields and destination suggestions, sends it through the existing Credential Broker trip-intelligence/Kimi route, preserves the new local trip id/source id, aligns returned itinerary days to the chosen trip dates, and stores the generated itinerary into both the active `TripProfile` and `customItinerary`. If AI/session/quota is unavailable or returns no useful spots, the wizard now falls back to destination-aware itinerary generation; Jeju/Korea fallback creates practical spots such as 城山日出峰、牛島、漢拏山、萬丈窟、東門市場、天地淵瀑布 with `lat/lon`, `city`, `country`, `Asia/Seoul`, and `KRW`, so Timeline and Weather have usable targets immediately. The trip create sync queue item now carries `tripId` and `sourceId`, allowing the existing Supabase/Personal Notion sync engine to create/update the active-trip backend record instead of leaving the new trip local-only. Verification passed: GitNexus impact on compact `Dashboard` LOW; GitNexus detect-changes LOW; `npm run typecheck`; `node scripts/run-with-dev-server.mjs -- npm run smoke:dashboard` (6 passed, including trip-intelligence wizard create, Jeju itinerary, Timeline, Weather target, KRW, and trip sync queue); `node scripts/run-with-dev-server.mjs -- npm run smoke:timeline` (7 passed); `node scripts/run-with-dev-server.mjs -- npm run smoke:weather` (12 passed after hardening the Weather smoke fixture against IndexedDB bleed-through); `npm run smoke:production-gate` (82.1s); `git diff --check`; manual Vercel production deploy; `npm run smoke:deploy-live` for deployment `dpl_895zhEKvsF7gNzCT86D56WcPTBPR`.
- Previous compact app-code proof commit: `c57f6ba` (`Add destination-aware trip wizard suggestions`).
- 2026-06-10 compact Home destination-aware new-trip wizard pass: Step 1 destination input now recognizes Korea/Jeju and auto-selects KRW for the main settlement currency. The wizard fetches destination attraction suggestions from public Wikivoyage/Wikipedia API endpoints with no API key, debounced from the destination field, then falls back to curated Jeju/Nagoya suggestions when online lookup is unavailable. Step 4 no longer shows hardcoded Nagoya/Tokyo inspiration; it reuses the destination-specific suggestions and can fill `旅程詳情` with Jeju spots such as 城山日出峰、牛島、漢拏山、萬丈窟、涉地可支、天地淵瀑布. Verification passed: GitNexus impact on `Dashboard` LOW; GitNexus detect-changes LOW; live Node probe against Wikivoyage search for Jeju; `npm run typecheck`; `node scripts/run-with-dev-server.mjs -- npm run smoke:dashboard` (6 passed, including mocked online Jeju suggestions, KRW, and Step 4 detail fill); `npm run smoke:production-gate` (84.1s); `git diff --check`; `npm run smoke:deploy-live` for deployment `dpl_EvC79RvgQ5KjNA6wUUaGfCr9CZ2o`.
- Previous compact app-code proof commit: `ce37372` (`Fix compact trip wizard day selector`).
- 2026-06-10 compact Home new-trip wizard fix: Step 2 now has a real `旅程日數` selector plus +/- controls so Boss can choose the number of days directly from the Home trip dropdown's `建立新旅程` flow. Changing the day count immediately updates `endDate`, changing the start date preserves the selected duration, and `addDaysToIsoDate()` uses UTC component math to avoid Asia timezone off-by-one errors that turned 10 days into 9 days. Verification passed: GitNexus impact on `Dashboard` LOW; GitNexus detect-changes LOW; `npm run typecheck`; `node scripts/run-with-dev-server.mjs -- npm run smoke:dashboard` (6 passed, including the new wizard regression); `npm run smoke:production-gate` (93.9s); `git diff --check`; `npm run smoke:deploy-live` for deployment `dpl_GusQKVAHVwrHy5bHhPG4tR7YgSPP`.
- Previous compact app-code proof commit: `3060b85` (`Declutter compact stats budget layout`).
- 2026-06-10 compact Stats declutter pass: Stats `預算使用分析` no longer shows the yellow center dot inside the budget compass. The budget story cards now keep only `Used percent` and `Remaining / day`; `Fairness by person` and `Category anomaly` were removed. The four summary cards (`圖表統計額`, `共同分帳額`, `私人/代付`, `待轉帳`) and their React component/imports were removed, and `TOP 10 支出` now appears directly below the budget analysis card before the remaining story cards. Verification passed: GitNexus impact on `Stats` LOW; GitNexus detect-changes LOW; `npm run typecheck`; `node scripts/run-with-dev-server.mjs -- npm run smoke:stats`; `npm run smoke:mobile-layout`; `npm run smoke:production-gate` (87.9s); `git diff --check`; `npm run smoke:deploy-live` for deployment `dpl_PHVAGw1KXvsUEKGUSo6BEyADKRbd`.
- Previous compact app-code proof commit: `09f9cfd` (`Add compact official weather router`).
- 2026-06-10 compact Weather multi-country official-provider router pass: Weather now resolves an official provider from itinerary country/region/city and coordinates before private WeatherAPI/Open-Meteo fallback. Browser-direct official providers now include Japan JMA, Singapore NEA/data.gov.sg, US NWS, and Canada MSC GeoMet. Official data remains the displayed provider while fallback data only fills missing hourly fields such as feels-like, UV, cloud cover, or wind gusts. Routing is country-first when itinerary country exists, so Canada does not get misclassified as US via `America/*` timezones and Korea/Jeju does not get misclassified as Japan by coordinate bounds. Verification passed: GitNexus impact on `fetchWeather` and `Weather` LOW; GitNexus detect-changes LOW; `npm run typecheck`; `node scripts/run-with-dev-server.mjs -- npm run smoke:weather` (12 passed); `npm run smoke:production-gate` (79.7s); contact sheet `/tmp/compact-contact-sheet-2026-06-10T06-37-43-338Z/mobile-contact-sheet.png`; `npm run smoke:deploy-live` for deployment `dpl_EJruZvEaK3fuh5NCy59WBDymDNrx`.
- 2026-06-10 compact Weather official-provider pass: Japan itinerary weather now tries true JMA official public JSON first (`www.jma.go.jp` forecast + AMeDAS observations) instead of letting broker-backed WeatherAPI run before official data. If JMA official lacks fields such as feels-like, UV, cloud cover, or wind gusts, compact fills only the missing fields from the broker-backed WeatherAPI forecast when a session exists, or from Open-Meteo/JMA model fallback otherwise. Fresh non-official cache entries are bypassed for Japan so private WeatherAPI/Open-Meteo cache cannot mask official JMA data. Verification passed: GitNexus impact on `fetchWeather` LOW; GitNexus detect-changes LOW; GitNexus refresh attempted but failed on the local FTS/index extension issue, so direct code/test evidence was used; `npm run typecheck`; `npm run smoke:weather` (10 passed); `npm run smoke:production-gate` (90.4s); contact sheet `/tmp/compact-contact-sheet-2026-06-10T06-01-57-899Z/mobile-contact-sheet.png`; `npm run smoke:deploy-live` for deployment `dpl_6SWopzxDZWJJNHjfF7GPTDyJ4YCq`.
- Weather provider policy: prefer official local meteorological data when the itinerary country has a safe source, then fall back to broker-backed commercial/private providers or Open-Meteo only for missing/unavailable fields. Do not put official provider API keys in the public frontend. Browser-direct official providers currently implemented: Japan JMA, Singapore NEA/data.gov.sg, US NWS, and Canada MSC GeoMet. Korea KMA, Taiwan CWA, UK Met Office, DWD, Meteo-France, BOM, and MET Norway production use should go through Credential Broker/backend proxy because of API keys, CORS, User-Agent/cache obligations, parser complexity, or licence constraints.
- Previous compact app-code proof commit: `bf5c8c4` (`Prefer official JMA weather in compact`).
- Previous compact app-code proof commit: `dabb152` (`Prevent compact weather metric truncation`).
- 2026-06-10 compact Weather metric readability fix: Weather slot metrics now use a 2x2 grid on mobile instead of four cramped columns, so UV/cloud, rain/precipitation, wind/gust, and humidity values are readable without `...`. UV/cloud remains the left-most/first chip and uses compact text such as `UV 6 · 雲35%`. Verification passed: screenshot proof `/tmp/weather-metrics-no-ellipsis-after.png` and `/tmp/weather-metrics-no-ellipsis-card-after.png`; geometry proof confirmed every live metric value had `scrollWidth <= clientWidth`; GitNexus impact on `Weather` LOW; `npm run typecheck`; `node scripts/run-with-dev-server.mjs -- npm run smoke:weather` (10 passed, including all live metric values fit); `git diff --check`; `npm run smoke:production-gate` (77.9s); contact sheet `/tmp/compact-contact-sheet-2026-06-10T05-30-34-728Z/mobile-contact-sheet.png`; `npm run smoke:deploy-live` for deployment `dpl_FobtW3gA4frBxXKg47xhU9M5hKW3`.
- Previous compact app-code proof commit: `7dddb74` (`Sync compact weather live preview`).
- 2026-06-10 compact Weather live-preview sync fix: the top current-weather card now chooses the same live time slot as the first location card instead of defaulting to the first forecast slot. This fixes cases where the Nagoya location card's `LIVE` slot showed 27°C while the top card still showed an earlier 22°C slot. Weather metric chips now render UV/cloud first, followed by rain, wind, and humidity, so the UV capsule sits on the left edge. Verification passed: GitNexus impact on `Weather` LOW and detect-changes HIGH because Weather render/test symbols changed; `npm run typecheck`; `node scripts/run-with-dev-server.mjs -- npm run smoke:weather` (10 passed, including top 27°C matching the live slot and first metric `sun-tag`); screenshot/geometry proof `/tmp/weather-live-temp-uv-after.png` and `/tmp/weather-live-temp-uv-card-after.png`; `git diff --check`; `npm run smoke:production-gate` (82.3s); contact sheet `/tmp/compact-contact-sheet-2026-06-10T04-56-46-156Z/mobile-contact-sheet.png`; `npm run smoke:deploy-live` for deployment `dpl_7Qf6LKbWF4f6n9bwr5XzQeujfvTt`.
- Previous compact app-code proof commit: `c18111b` (`Fix compact weather card overlays`).
- 2026-06-10 compact Weather card overlay fix: Weather location cards no longer let the decorative accent rail cross over the first row of slot text. The slot accent line is pinned to the card top edge, decorative glow layers sit behind content, and slot content has a higher stacking layer. Weather provider UI now displays broker-backed private weather as `Live weather` and sanitizes fallback copy so `WeatherAPI.com` does not appear in Weather cards. Verification passed: screenshot proof before `/tmp/weather-region-card-before.png` and after `/tmp/weather-region-card-after.png`; geometry proof confirmed the accent line no longer overlaps the header and `WeatherAPI.com` is absent; GitNexus impact on `Weather` LOW and detect-changes HIGH because Weather render labels changed; `npm run typecheck`; `node scripts/run-with-dev-server.mjs -- npm run smoke:weather` (10 passed); `node scripts/run-with-dev-server.mjs -- npm run smoke:mobile-layout`; `npm run build`; `npm run security:scan`; `git diff --check`; `npm run smoke:production-gate` (74.3s); contact sheet `/tmp/compact-contact-sheet-2026-06-10T04-21-16-745Z/mobile-contact-sheet.png`; `npm run smoke:deploy-live` for deployment `dpl_F2Mm5VwZQmKSm9LL63jQDfL6WspG`.
- Previous compact app-code proof commit: `f71fbff` (`Fix compact weather itinerary forecasts`).
- 2026-06-09 compact Weather itinerary/current-weather fix: Weather's top current-weather card now uses a dedicated inner layout so provider/live/target/fallback chips, actual temperature, feels-like temperature, high/low/humidity/wind facts, place label, and hourly chips fit inside the large card on 390px mobile. Finished trips no longer collapse all itinerary locations into one synthetic today card; Weather keeps the original itinerary day cards and fetches current weather for each day location using today's date after the trip ends. Day/location resolution now uses up to six itinerary-derived coordinates per day, so multi-spot days and post-trip current weather are not limited to two locations. Verification passed: GitNexus impact on `Weather` LOW and `coordsForDay` MEDIUM before editing; GitNexus detect-changes reported HIGH because the `Weather` render/load flow changed, mitigated by `npm run typecheck`, `node scripts/run-with-dev-server.mjs -- npm run smoke:weather` (10 passed), `node scripts/run-with-dev-server.mjs -- npm run smoke:mobile-layout`, `npm run build`, `npm run security:scan`, `git diff --check`, `npm run smoke:production-gate` (132.3s), mobile screenshot proof `/tmp/compact-weather-top-card-after-3.png`, contact sheet `/tmp/compact-contact-sheet-2026-06-09T15-52-45-326Z/mobile-contact-sheet.png`, and live deploy proof `npm run smoke:deploy-live` for deployment `dpl_81o32cF6fgRwNYux3VKuFcMxNAHf`.
- Previous compact app-code proof commit: `ff24cbc` (`Declutter compact records and timeline controls`).
- 2026-06-09 compact Records/Timeline cleanup: History/Record no longer renders the `Review only when needed` shortcut strip or the old itinerary/cleanup review derived state. Record rows, search/category filters, conflict resolver, pending confirmation flow, and Settings Trip Doctor markers remain. Compact receipt normalization now self-heals HK Express / 香港快運 / `UO` flight-number receipts from `transport` or `other` into the `flight` category, so existing HK Express records appear under `機票`. Timeline no longer renders the travel-day widget/readiness/countdown row; the live itinerary rail, command card, day cards, and `Weather pack` strips remain. Verification passed: GitNexus impact on `History` and `Timeline` LOW; `categoryIdFromName` HIGH and `stampReceiptForTrip` CRITICAL were mitigated by narrow HK Express-only normalization plus smokes; `npm run typecheck`; `node scripts/run-with-dev-server.mjs -- npm run smoke:history` (8 passed); `node scripts/run-with-dev-server.mjs -- npm run smoke:timeline` (7 passed); targeted Notion mapping smoke (8 passed); `node scripts/run-with-dev-server.mjs -- npm run smoke:final-nav` (8 passed); `node scripts/run-with-dev-server.mjs -- npm run smoke:mobile-layout`; `npm run build`; `npm run security:scan`; `git diff --check`; `npm run smoke:production-gate` (81.8s); `npm run smoke:deploy-live` for deployment `dpl_FAEsqTEXmDp3TDzu9MAskvQaFREz`.
- Previous compact app-code proof commit: `b69887e` (`Compact Home information density pass`).
...
- 2026-06-09 compact Home information-density pass: Dashboard/Home no longer renders `Travel-day control`, `Itinerary Receipt Match`, `Trip Snapshot`, `Departure Checklist`, `Day-end Closeout`, bottom `旅程提醒`, Today Situation `預算分析` / `行程時間線` actions, `Local AI Coach`, or the `預算控制` accordion. The Today Itinerary card now uses compact itinerary-style rows with time, icon, place, note/address, city/type, and matched receipt/map action; Recent Expenses now shows 6 compact rows. Shared `travelDay` helpers remain for Timeline and History, and the broker-backed Dashboard AI assistant remains available. Verification passed: GitNexus impact on `Dashboard` was LOW before editing; GitNexus detect-changes reported HIGH because `Dashboard` appears in 6 indexed flows; `npm run typecheck`; `node scripts/run-with-dev-server.mjs -- npm run smoke:dashboard` (5 passed); `node scripts/run-with-dev-server.mjs -- npm run smoke:mobile-layout`; `npm run smoke:contact-sheet`; `npm run build`; `npm run security:scan`; `git diff --check`; `npm run smoke:production-gate` (111.7s); `npm run smoke:deploy-live` for deployment `dpl_BfhqqyhVNaUi6fSCfKEi9Dw9DhUa`.
- Previous compact app-code proof commit: `a1d2fe2` (`Simplify compact home dashboard`).
- 2026-06-09 compact Record declutter pass: History/Record no longer shows large `Itinerary Review Queue`, `Attachment Health`, or `Cleanup Coach` cards above the receipt list. A later same-day pass also removed the compact `Review only when needed` shortcut strip and its derived review state, leaving Records focused on search/filter/list, row-level health markers, pending confirmations, and conflict resolution. Attachment/photo health remains summarized in Settings `Compact Trip Doctor` as an `Attachments` item, while individual Record rows keep small `photo large`, `photo missing`, and `photo unsynced` markers. Verification passed: GitNexus impact on `History` and `Settings` was LOW; `npm run typecheck`; `node scripts/run-with-dev-server.mjs -- npm run smoke:history` (8 passed); `node scripts/run-with-dev-server.mjs -- npm run smoke:settings` (8 passed); `node scripts/run-with-dev-server.mjs -- npm run smoke:final-nav` (8 passed); `npm run smoke:production-gate` (83.7s); `npm run smoke:deploy-live` for deployment `dpl_2ZEBiwXihz3VtxsQMQwuEYhzWG93`.
- Previous compact app-code proof commit: `22bb1f2` (`Declutter compact record diagnostics`).
- Previous compact app-code proof commit: `e1fa760` (`Keep compact readiness capsules in settings`).
...
- P0-05 remains `LIVE`: verifying live Notion/Kimi/Google/Gemma/Mimo/WeatherAPI broker-vault paths requires live provider/account state and must not print secrets.
- Latest authenticated P0-05 evidence: Boss created a valid ignored local broker session with `npm run broker-vault:prepare`; `npm run smoke:broker-vault:doctor` reports the session file is present, git-ignored, permission `0600`, and unexpired. The full broker-vault proof now continues after provider failures and reports redacted JSON instead of a stack trace. It treats Notion database proof as covered by `/credentials/test-all` without printing database ids, and separately probes a Google diagnostic model for route/credential health without changing compact's primary model contract.
- Current authenticated P0-05 result on 2026-06-10 HKT: `npm run smoke:broker-vault:doctor` reports the ignored local session is ready (`0600`, unexpired). `npm run smoke:broker-live` passes live health/CORS/no-session guards. After the Cloudflare deploy permission fix and Worker deploy, a focused authenticated `/mimo/json` proof returned HTTP 200 with provider/model `mimo-v2.5`. Full `npm run smoke:broker-vault` still exits `2` with clean redacted JSON because Kimi is account-limited for the current billing cycle. Passed portions now include broker session auth, `/credentials/status`, Notion/database permission via `/credentials/test-all`, WeatherAPI forecast with hourly and feels-like data, Google JSON, Google diagnostic JSON, and Mimo JSON. Do not bypass Kimi quota by falling back after provider limit errors.
- Credential Broker deploy preflight for P0-05: from `workers/credential-broker/`, run `npm run preflight:deploy`. It performs no-secret source/dry-run/auth/live checks. Current 2026-06-10 evidence: source contains `/mimo/json`, worker syntax/self-test pass, Wrangler dry-run bundles, Wrangler OAuth auth passes, live health passes, and live `/mimo/json` unauthenticated guard returns 401. Worker production deploy succeeded with version `7524c7dd-428a-4c11-bd75-bc6ebffeb20e`.
- Broker-vault guard hardening: `npm run smoke:broker-vault:guard` now forces missing-session mode even if an ignored local proof session exists, so normal release gates do not accidentally spend provider quota or call provider endpoints.
- Latest compact smoke hardening: `npm run smoke:security` now owns/reuses the Vite dev server through `scripts/run-with-dev-server.mjs`, using a restricted child environment. This fixes standalone `ERR_CONNECTION_REFUSED` failures when no compact dev server is already running.
...
- Latest P0-05 preflight evidence: 2026-06-09 re-audit found no ignored `.broker-vault-session.local.json` in repo/app roots. `npm run smoke:broker-live` passed with `/health` 200 (`travel-expense-credential-broker`, version `2026.05.29`), CORS preflight 204 for `https://travel-expense-compact.vercel.app`, protected Notion/Kimi/Google/Mimo/WeatherAPI/credentials paths 401 `Session missing`, and no sensitive-looking response text. `npm run smoke:broker-vault:guard` also passed, proving the authenticated proof workflow fails closed without a local session. A normal `npm run smoke:broker-vault` run safely stopped with `status: blocked` / `missing local broker vault session`; provider vault calls were not executed. `npm run smoke:broker-vault:doctor` now exists to check ignored auth input presence, git-ignore coverage, private permissions, auth mode, and expiry without printing values or calling providers. `npm run broker-vault:prepare` now creates an ignored private session file from a hidden local terminal prompt. This does not prove provider vault contents; authenticated provider tests still require a safe ignored local broker session or Supabase token via `npm run smoke:broker-vault`.
...
- 2026-06-08 compact P2 progress:
  - P2-01 historical note: Dashboard previously had a local-only `Local AI Coach` panel for daily burn, overspend forecast, next-day warning, and weather-linked reminders. Boss later asked to simplify Home, so the `Local AI Coach` Home panel is now removed; keep the broker-backed Dashboard AI assistant, and keep relevant weather/timeline logic outside Home.
  - Current P2-01 verification after retirement: `npm run typecheck`, `node scripts/run-with-dev-server.mjs -- npm run smoke:dashboard` (5 passed), `node scripts/run-with-dev-server.mjs -- npm run smoke:mobile-layout`, `npm run smoke:contact-sheet`, `npm run build`, `npm run security:scan`, `git diff --check`, and `npm run smoke:production-gate` (111.7s) passed from `app-compact/`.
...
- 2026-06-08 compact P4 progress:
  ...
  - P4-02 follow-up for P0-05: Compact now also has `npm run smoke:broker-vault:doctor`, a no-provider-call readiness check that reports `blocked`, `attention`, or `ready` from ignored session file/env presence, git-ignore coverage, permissions, auth mode, and expiry. It never prints session/token values. Missing-session and fake-env ready paths were verified.
  ...
  - P5-02 done: Welcome Guide now captures first-run personalization for trip style, preferred trip currency, home city, and weather preference, then stores it in trip intelligence. Settings now exposes the same trip personalization fields for later edits. App boot now treats an existing local Supabase session as a real `supabase:<user_id>` storage scope, preventing public new users from falling back to legacy local/demo Nagoya state when the Supabase SDK session is not hydrated yet.
  - Verification for P5-02: added `npm run smoke:welcome-guide`, which seeds a new public Supabase user, saves KRW/food/rain/Hong Kong personalization, reloads Settings, verifies the persisted trip-intelligence fields, and confirms no `trip_2026_04_nagoya`, `trip_default`, or legacy receipts leak into the new account. Additional checks passed: `npm run smoke:settings`, `npm run smoke:security`, `npm run smoke:auth-broker`, `npm run smoke:mobile-layout`, `npm run smoke:a11y-touch`, `npm run smoke:weather`, `npm run smoke:timeline`, `npm run smoke:stats`, `npm run smoke:trip-intelligence`, `npm run smoke:production-gate`, `npm run build`, `npm run typecheck`, and `git diff --check`.
  - GitNexus `detect-changes` reported CRITICAL because `normalizeTripIntelligence` / trip context is shared by Weather, Timeline, Stats, Settings, Supabase, and Notion flows. The extra Weather/Timeline/Stats/Trip Intelligence/Security/Auth smokes above were run to cover that blast radius.
  - P5-03 done: Compact now has `npm run smoke:shared-contract`, a compact-owned cross-version compatibility smoke that starts compact and React Vite servers, seeds one public-safe shared fixture into `boss-japan-tracker`, disables external secret/provider paths, and compares the normalized shared storage contract across trips, people, share ratios, receipts, settings, sync queue metadata, Supabase IDs, Notion metadata, and common trip-intelligence fields.
  - The shared-contract smoke treats compact schema v4 and React schema v3 as compatible shared-contract versions, while separately proving compact-only personalization fields (`tripStyle`, `homeCity`, `weatherPreference`) survive compact boot. It intentionally keeps `autoSync` off in the fixture so the comparison checks data-contract compatibility instead of React boot-sync side effects.
  - Verification for P5-03: `npm run smoke:shared-contract` passed with compact schema `4`, React schema `3`, `1` trip, `2` receipts, `3` persons, and `3` sync queue items compared. Broader release verification also passed: `npm run typecheck`, `npm run build`, `git diff --check`, and `npm run smoke:production-gate:full` (217.9s, including the new shared-contract smoke).
  - P5-04 historical note: Compact previously had shared `buildTravelDayWidgets()` logic for transit countdown, receipt reminder, weather alert, and next booking note. Boss later removed the Dashboard/Home panel and then removed the Timeline travel-day widget/readiness/countdown row as well; `travelDay.ts` now keeps only the weather packing-risk helper used by Timeline day cards.
  ...
  - P6-03 historical note: Compact History previously added cleanup suggestions for Pending OCR, Duplicate SourceID, Missing photo, and Missing payer. The old large `Cleanup Coach` panel was first superseded by the compact `Review only when needed` strip, then Boss asked to remove that strip too. Records now keeps row markers, pending confirmations, conflict resolver, search, and category filters without a cleanup shortcut surface.
  - Verification for P6-03: targeted RED History smoke first failed on missing cleanup suggestions, then passed. Latest current-layout proof includes `npm run smoke:history` (8 passed), `npm run smoke:settings` (8 passed), and `npm run smoke:final-nav` (8 passed).
  ...
  - Follow-up hardening in the same compact pass: `StatefulActionButton` animation feedback now fails closed if a Scan/Settings action unmounts the button before Motion finishes, preventing the observed `querySelectorAll` unhandled rejection. `smoke:shared-contract` now compares stable shared trip/receipt/person/settings identity fields while reporting compact-vs-React runtime sync differences separately, because React can legitimately clear queued sync items during boot.
  - Verification for the follow-up hardening: `node --check scripts/shared-contract-smoke.mjs`, standalone `npm run smoke:shared-contract`, `npm run typecheck`, `git diff --check`, and `npm run smoke:production-gate:full` passed. The final full gate passed in 280.1s; latest contact sheet: `/tmp/compact-contact-sheet-2026-06-08T10-40-50-095Z/mobile-contact-sheet.png`.
  - P7 roadmap added: `app-compact/COMPACT_IMPROVEMENT_CHECKLIST.md` now tracks next travel-reliability improvements for booking-reference staleness, offline conflict resolution, receipt-photo/attachment health, per-day readiness scoring, and local release-note diffs.
  - P7-01 done: Dashboard and Timeline travel-day widgets now warn `Booking stale` when the next upcoming booking receipt has a `bookingRef` but its `updatedAt` is older than 30 days. The widget keeps the booking age, ref, store, and time visible so Boss can still check the actual reservation detail before transport/ticket/hotel checkpoints.
  - App-root cleanup during P7-01: the expected Welcome Guide no-session fallback now saves the new trip locally without logging `Failed to save guide trip: Supabase session unavailable` as a console error.
  - Verification for P7-01: targeted Dashboard/Timeline booking-stale smokes passed, then broader checks passed: `npm run smoke:dashboard` (8 passed), `npm run smoke:timeline` (10 passed), `npm run smoke:welcome-guide`, `npm run smoke:final-nav` (8 passed), `npm run typecheck`, `npm run smoke:mobile-layout`, `npm run smoke:contact-sheet`, `npm run build`, `npm run security:scan`, and `npm run smoke:production-gate:full` (255.5s). Latest contact sheet: `/tmp/compact-contact-sheet-2026-06-08T15-46-16-864Z/mobile-contact-sheet.png`.
  - P7-02 done: Compact History now has an `Offline Conflict Resolver` panel for current-trip receipt conflicts from failed receipt sync queue items or receipt-level `error`/`failed` sync status. It gives users three safe actions: `Review conflict` opens the receipt editor, `Keep local` sanitizes the queue payload and requeues the local receipt without provider data, and `Keep cloud` clears the failed local retry while trusting the existing cloud copy.
  ...
  - P7-03 done: Compact receipt photo quality checks detect oversized local images, missing expected OCR/email attachments, and local photos that are not yet backed by cloud-safe attachment state. Current UI note: the old large History `Attachment Health` panel is superseded by the 2026-06-09 Record declutter pass; Settings `Compact Trip Doctor` now summarizes `Attachments`, while receipt rows keep `photo large`, `photo unsynced`, and `photo missing` markers.
  ...
  - P7-04 historical note: Compact previously had per-day readiness scoring through `buildDayReadinessScores()` in `app-compact/src/lib/travelDay.ts`. Boss later removed the Home readiness strip and then removed the Timeline readiness/countdown row; that helper and UI are now retired from compact.
  - P7-04 retired scoring inputs were itinerary coverage, active-trip route freshness, cached/local weather freshness and weather risk, stale booking references older than 30 days, completed-spot receipt gaps up to the actual time of day, and cleanup markers. Keep this as history only; do not reintroduce the row unless Boss asks.
  - Verification for P7-04: targeted Dashboard/Timeline RED smokes first failed on missing `Day readiness scores`, then passed after implementation. Broader checks passed: `npm run typecheck`, `npm run smoke:dashboard` (9 passed), `npm run smoke:timeline` (11 passed), `npm run smoke:mobile-layout`, `npm run smoke:contact-sheet`, `npm run build`, `npm run security:scan`, and final `npm run smoke:production-gate:full` (243.5s). Latest contact sheet: `/tmp/compact-contact-sheet-2026-06-08T16-53-13-213Z/mobile-contact-sheet.png`.
  - P7-05 done: Compact Shell now exposes a local release-note diff panel when the PWA readiness strip reaches `Update · ready`. Users can tap `Release notes` to see a short `Compact release notes / Now vs previous` summary covering day readiness scores, attachment checks, and offline conflict resolution. The panel is local/static, dismissible via `Done`, and stores only a local seen marker.
  - P7-05 no-secret/no-network behavior: final-nav smoke watches for `github`, `changelog`, `release-notes`, or `releases` network requests after update-ready and confirms none are made. The panel does not fetch GitHub, read remote changelogs, call broker/provider APIs, or expose credentials.
  - Verification for P7-05: targeted RED final-nav smoke first failed on missing `Release notes`, then passed after implementation. Broader checks passed: `npm run typecheck`, `npm run smoke:final-nav` (8 passed), `npm run smoke:mobile-layout`, `npm run smoke:contact-sheet`, `npm run build`, `npm run security:scan`, and final `npm run smoke:production-gate:full` (266.3s). Latest contact sheet: `/tmp/compact-contact-sheet-2026-06-08T17-06-39-639Z/mobile-contact-sheet.png`.
  - P8 roadmap added: `app-compact/COMPACT_IMPROVEMENT_CHECKLIST.md` now tracks travel-handoff and companion-flow tasks for local trip snapshots, weather/itinerary packing checks, day-end closeout, and private trip-share export previews.
  - P8-01 done: Dashboard now has a local/no-API `Trip Snapshot` card showing Day, Budget left, Next, and Watch signals, plus copyable summary text and Timeline/Records shortcuts. It reads existing trip, budget, travel-day widget, readiness, and receipt context only; it does not call broker/provider APIs, write storage, add schema fields, or expose credentials.
  - Verification for P8-01: targeted `npm run smoke:dashboard` passed with 10 tests, including mocked clipboard copy, local/no-API text, and Timeline/Records navigation checks. Broader checks passed: `npm run typecheck`, `npm run security:scan`, `npm run smoke:mobile-layout`, `npm run smoke:a11y-touch`, `npm run smoke:contact-sheet`, `npm run build`, and final `npm run smoke:production-gate` (99.4s). Manual Vercel production deploy from linked `app-compact/` project produced `dpl_DnjLXzyypyXV9uSXsc798LAqAtC3`; `npm run smoke:deploy-live` passed for commit `810d3c9`.
  - P8-02 done: Dashboard now has a local/no-API `Departure Checklist` card that derives Weather kit, Route kit, Itinerary type, Booking check, and Receipt habit from existing travel-day widgets, current itinerary spots, readiness issues, and booking/receipt signals. It does not call AI/broker/provider APIs, write storage, add schema fields, or expose credentials.
  - Verification for P8-02: targeted `npm run smoke:dashboard` passed with 10 tests, including rain/outdoor/transport/booking/receipt-gap checklist coverage and Weather shortcut navigation. Broader checks passed: `npm run typecheck`, `npm run security:scan`, `npm run smoke:mobile-layout`, `npm run smoke:a11y-touch`, `npm run smoke:contact-sheet`, `npm run build`, and final `npm run smoke:production-gate` (82.6s). Latest contact sheet: `/tmp/compact-contact-sheet-2026-06-09T02-51-26-794Z/mobile-contact-sheet.png`. Manual Vercel production deploy from linked `app-compact/` project produced `dpl_RjHoxECCHK2BYckYMg2UkEyrttRE`; `npm run smoke:deploy-live` passed for commit `1efcf4f`.
  - P8-03 done: Dashboard now has a local/no-API `Day-end Closeout` card for evening wrap-up. It summarizes missing receipts for completed itinerary spots up to the actual time of day, daily budget overrun notes, tomorrow readiness, and cleanup action, then links directly to Records, Stats, and Tomorrow's Timeline.
  - P8-03 safety behavior: the closeout card reads existing itinerary, receipt, budget, and readiness state only. It does not write storage, add schema fields, call broker/provider APIs, expose credentials, or change React/legacy behavior.
  - Verification for P8-03: targeted `npm run smoke:dashboard` passed with 11 tests, including missing receipt, daily budget note, tomorrow readiness, and Records/Stats/Tomorrow navigation coverage. Broader checks passed: `npm run typecheck`, `npm run security:scan`, `npm run smoke:mobile-layout`, `npm run smoke:a11y-touch`, `npm run smoke:contact-sheet`, `npm run build`, `git diff --check`, and final `npm run smoke:production-gate` (137.7s). Latest contact sheet: `/tmp/compact-contact-sheet-2026-06-09T03-09-14-189Z/mobile-contact-sheet.png`. Historical 2026-06-09 `npm run smoke:broker-vault` evidence was blocked by Kimi quota plus then-unfixed Google/Mimo live state; the Google/Mimo/Mimo-vault portions were superseded by the 2026-06-10 Cloudflare deploy and broker proof, leaving Kimi quota as the live blocker. Manual Vercel production deploy from linked `app-compact/` project produced `dpl_CxWB3mbtgL4PhkfvynRqvncdGsF8`; `npm run smoke:deploy-live` passed for commit `20568db` with asset hash `5584addcbe22acc9`.
  ...
  - P9-02 done: Settings now has `Post-trip Archive`, a local/no-API finish checklist for trips whose end date has passed or that are already archived. It separates final Backup JSON, private trip-share preview, settlement review, and clear-local-data preview into four actions; it does not directly archive, delete, call providers, add schema fields, or touch React/legacy behavior.
  - Verification for P9-02: targeted `npm run smoke:settings -- --grep "post-trip archive"` passed through `scripts/run-with-dev-server.mjs`; full `npm run smoke:settings` passed with 6 tests. Broader checks passed: `npm run typecheck`, `npm run security:scan`, `npm run build`, `npm run smoke:mobile-layout`, `npm run smoke:a11y-touch`, `npm run smoke:contact-sheet`, `git diff --check`, and final `npm run smoke:production-gate` (75.9s). Latest contact sheet: `/tmp/compact-contact-sheet-2026-06-09T03-49-41-567Z/mobile-contact-sheet.png`. Manual Vercel production deploy from linked `app-compact/` project produced `dpl_ByavGUVve1btyEdedjkvCb4U7Q7w`; `npm run smoke:deploy-live` passed for commit `c122922` with asset hash `16977b858e54e2a9`.
  - P9-03 done: Settings `Notion Sync` now has a local-only `Sync dry run` panel before push controls. It summarizes active-trip pending queue, receipt/trip/settings operation mix, failed/conflict signals, oldest offline edit age, last sync age, push target, and delete-queued warnings. It intentionally does not call broker, Supabase, Notion, or provider APIs, and does not render queue error strings or provider-looking secret payloads.
  - Verification for P9-03: targeted `npm run smoke:settings -- --grep "sync readiness dry run"` passed through `scripts/run-with-dev-server.mjs`; full `npm run smoke:settings` passed with 7 tests. Broader checks passed: `npm run typecheck`, `npm run security:scan`, `npm run build`, `npm run smoke:mobile-layout`, `npm run smoke:a11y-touch`, `npm run smoke:contact-sheet`, `git diff --check`, and final `npm run smoke:production-gate` (83.2s). Latest contact sheet: `/tmp/compact-contact-sheet-2026-06-09T04-03-47-468Z/mobile-contact-sheet.png`. Manual Vercel production deploy from linked `app-compact/` project produced `dpl_3Hq2eTA4cFZDNpb5UgG9RSbFD5BU`; `npm run smoke:deploy-live` passed for commit `8b4a2c0` with asset hash `7c0ef4f8f3a302cd`.
  - P10 roadmap added for trip-scope and data-integrity work: active-trip scope audit, receipt date/trip repair shortcut, day-by-day itinerary/receipt reconciliation, and public-safe diagnostics bundle preview.
  - P10-01 done: Settings now has a top-level `Trip Scope Audit` card before export/share/backup/sync decisions. It summarizes active-trip included receipt count, date-window outliers, auto-linked originally-unlinked receipts, and other-trip exclusions. Compact receipt normalization now preserves a `tripLinkSource` marker (`explicit`, `date-auto`, `prep-auto`, or `fallback-auto`) so repeated migrations stay idempotent while the audit can still surface auto-linked receipts for review.
  - P10-01 safety behavior: the audit is local/no-API, does not call broker/Supabase/Notion/provider endpoints, and does not render SourceID, Notion IDs, Supabase IDs, queue errors, photo URLs, or provider-looking secrets. The current-trip export/sync scope remains unchanged; auto-linked receipts are included but flagged for review.
  - Verification for P10-01: targeted `npm run smoke:settings -- --grep "trip scope audit"` first exposed the normalizer auto-link/idempotency issue, then passed after preserving `tripLinkSource`. Broader checks passed: `npm run typecheck`, `npm run smoke:shared-contract`, full `npm run smoke:settings` (8 passed), `npm run smoke:history` (7 passed), `npm run smoke:stats`, `npm run smoke:mobile-layout`, `npm run smoke:a11y-touch`, `npm run smoke:contact-sheet`, `npm run security:scan`, `npm run build`, `git diff --check`, and final `npm run smoke:production-gate` (77.1s). Latest contact sheet: `/tmp/compact-contact-sheet-2026-06-09T04-21-35-044Z/mobile-contact-sheet.png`. GitNexus `detect-changes` reported HIGH because `stampReceiptForTrip` and `Settings` are shared execution-flow symbols; the broader smoke set above is the release proof. Manual Vercel production deploy from linked `app-compact/` project produced `dpl_3YGbzZGF5B1K8mAik7WmSkvbE4Qo`; `npm run smoke:deploy-live` passed for commit `78a7d67` with asset hash `460dc10c94b79fb5`.
  - P10-02 done: Settings `Trip Scope Audit` now offers `Repair first issue` when the audit finds a date-window outlier or auto-linked receipt that needs review. The shortcut writes a one-shot `sessionStorage` repair intent, jumps to History, and History opens the matching receipt editor before clearing the intent. Date-window outliers are prioritized before auto-linked receipts.
  - P10-02 safety behavior: the repair shortcut is local-only and session-only. It adds no schema fields, persists no repair state into app data, calls no broker/Supabase/Notion/provider APIs, and exposes no secrets or internal IDs.
  - Verification for P10-02: targeted `npm run smoke:settings -- --grep "trip scope audit"` passed through the dev-server wrapper; full `npm run smoke:settings` passed with 8 tests and `npm run smoke:history` passed with 7 tests. Broader checks passed: `npm run typecheck`, `npm run security:scan`, `npm run build`, `git diff --check`, `npm run smoke:mobile-layout`, `npm run smoke:a11y-touch`, standalone `npm run smoke:contact-sheet`, and final `npm run smoke:production-gate` (75.8s). Latest contact sheet: `/tmp/compact-contact-sheet-2026-06-09T04-34-29-342Z/mobile-contact-sheet.png`. A first parallel contact-sheet/a11y attempt hit a shared dev-server port race; rerunning contact-sheet alone passed. GitNexus `detect-changes` reported HIGH because `Settings` and `History` are shared execution-flow symbols; the listed broader smoke set is the release proof. Manual Vercel production deploy produced `dpl_Buc32pf6nDJvRA7FU54u3LGii32p`; `npm run smoke:deploy-live` passed for commit `df1e501` with asset hash `1d97559a72251c6a`.
  - P10-03 historical note: Dashboard previously had an `Itinerary Receipt Match` / `每日紀錄對帳` card backed by `buildItineraryReceiptReconciliation()` in `app-compact/src/lib/travelDay.ts`. Boss removed the Home card first, then removed the Records `Review only when needed` shortcut strip and the shared reconciliation helper. Keep itinerary/receipt reconciliation as retired history unless Boss asks to rebuild it.
  - P10-03 safety behavior: the reconciliation is local/no-API and reads existing itinerary/receipt state only. It adds no schema fields, writes no storage, calls no broker/Supabase/Notion/provider endpoints, and renders no SourceID, cloud IDs, queue errors, photo URLs, tokens, or secrets.
  - Verification for P10-03: GitNexus impact for `Dashboard` and `buildDayReadinessScores` was LOW; `npx gitnexus analyze` is currently blocked by missing `tree-sitter-swift` in the GitNexus CLI dependency set, so the existing index was used and `npx gitnexus detect-changes --scope unstaged -r travel-expense` reported HIGH because `Dashboard` / `travelDay` affect shared execution flows. Release proof passed: `npm run typecheck`, targeted `npm run smoke:dashboard -- --grep "reconciles itinerary days"`, full `npm run smoke:dashboard` (12 passed), `npm run build`, `npm run security:scan`, `git diff --check`, `npm run smoke:mobile-layout`, `npm run smoke:a11y-touch`, `npm run smoke:contact-sheet`, and final `npm run smoke:production-gate` (75.7s). Latest contact sheet: `/tmp/compact-contact-sheet-2026-06-09T04-50-00-467Z/mobile-contact-sheet.png`. Manual Vercel production deploy produced `dpl_HjZgAoUwDLXTQAZ6Xee5egAzbJqE`; `npm run smoke:deploy-live` passed for commit `2cdb1d5` with asset hash `8a0b517705ad942b`.
  - P10-04 done in code and pushed: Settings `資料管理 / Security` now has `Preview diagnostics`, opening a `Public diagnostics preview` with copy and safe JSON download actions. The diagnostics payload is aggregate-only: compact surface/version, storage/sync modes, date/count summaries, category counts, queue counts, and safe checks. It intentionally excludes raw trips, raw receipts, raw people, raw sync queue, IDs, SourceID, queue payloads/errors, traveller names, receipt/store names, photos, photo URLs, provider tokens, API keys, and broker sessions.
  - P10-04 safety behavior: the diagnostics preview is local/no-API, writes no storage, adds no schema fields, calls no broker/Supabase/Notion/provider endpoints, and does not read provider secrets beyond boolean session presence.
  - Verification for P10-04: GitNexus impact for `Settings` was LOW; index refresh remains blocked by the missing `tree-sitter-swift` GitNexus CLI dependency. `npx gitnexus detect-changes --scope unstaged -r travel-expense` reported HIGH because `Settings` affects shared execution flows. Release proof passed: `npm run typecheck`, targeted `npm run smoke:settings -- --grep "expandable cards"` with fake secret/cloud/photo strings across preview/copy/download, full `npm run smoke:settings` (8 passed), `npm run build`, `npm run security:scan`, `git diff --check`, `npm run smoke:mobile-layout`, `npm run smoke:a11y-touch`, `npm run smoke:contact-sheet`, and final `npm run smoke:production-gate` (83s). Latest contact sheet: `/tmp/compact-contact-sheet-2026-06-09T05-04-17-029Z/mobile-contact-sheet.png`.
  - P10-04 deployment status: commit `8fadfcb` is pushed to `origin/main`, but Vercel production deploy is blocked by account daily API deploy quota (`api-deployments-free-per-day`). `npm run smoke:deploy-live` still reports the previous READY production deployment `dpl_EGaWZXC84K1MJAXSkdkBSjbnyFLm`; do not treat that as P10-04 live proof. Retry deployment after quota reset.
  - P11 roadmap added for decision support and trip review: Stats settlement action plan, History reconciliation review queue, Timeline weather packing-risk summary, and optional compact-maintainer release-recovery note.
  - P11-01 done in local code: Stats `分帳結算` now starts with a compact `Settlement action plan` showing `Next action`, `Total to settle`, and `Private repay` cards from existing settlement data. It exposes full traveller names, writes no storage, adds no schema fields, and calls no broker/provider APIs.
  - Verification for P11-01: GitNexus impact for compact `Stats` was LOW. Targeted `node scripts/run-with-dev-server.mjs -- npm run smoke:stats` passed with next-action, transfer total, private repay, full-name, HKD conversion, and 390px geometry coverage. Broader checks passed: `npm run typecheck`, `npm run build`, `npm run security:scan`, `node scripts/run-with-dev-server.mjs -- npm run smoke:mobile-layout`, `node scripts/run-with-dev-server.mjs -- npm run smoke:a11y-touch`, `npm run smoke:contact-sheet`, `git diff --check`, and final `npm run smoke:production-gate` (75.9s); latest contact sheet is `/tmp/compact-contact-sheet-2026-06-09T05-19-11-591Z/mobile-contact-sheet.png`. GitNexus detect-changes reported HIGH because compact `Stats` affects shared execution flows; the listed release proof is the risk check.
  - P11-01 deployment status: commit `e68f10c` is pushed to `origin/main`, but Vercel production deploy is still blocked by account daily API deploy quota (`api-deployments-free-per-day`). Live production remains the previous READY deployment `dpl_EGaWZXC84K1MJAXSkdkBSjbnyFLm`; do not treat `npm run smoke:deploy-live` as proof that P10-04 or P11-01 is live until a new deployment succeeds.
  - P11-02 retired: History previously had itinerary receipt review actions backed by `buildItineraryReceiptReconciliation()` logic. Boss later removed both the large review queue and the compact `Review only when needed` shortcut strip, so Records no longer renders missing-day/high-count/outside-itinerary review shortcuts.
  - P11-02 safety behavior: the review queue is local/no-API and reuses existing itinerary/receipt state only. It writes no storage, adds no schema fields, calls no broker/Supabase/Notion/provider endpoints, and renders no SourceID, cloud IDs, queue errors, photo URLs, tokens, or secrets.
  - Verification for P11-02: GitNexus impact for compact `History` was LOW, and detect-changes reported MEDIUM because `History` affects shared execution flows. Targeted `node scripts/run-with-dev-server.mjs -- npm run smoke:history` passed with 8 tests, including missing/high/outside itinerary review filtering and 390px geometry. Broader checks passed: `npm run typecheck`, `npm run build`, `npm run security:scan`, `git diff --check`, `node scripts/run-with-dev-server.mjs -- npm run smoke:mobile-layout`, `node scripts/run-with-dev-server.mjs -- npm run smoke:a11y-touch`, `npm run smoke:contact-sheet`, standalone `node scripts/run-with-dev-server.mjs -- npm run smoke:final-nav` after one transient final-nav timing miss, and final `npm run smoke:production-gate` (86.2s); latest contact sheet is `/tmp/compact-contact-sheet-2026-06-09T05-34-26-023Z/mobile-contact-sheet.png`.
  - P11-02 deployment status: commit `1f75aba` is pushed to `origin/main`, but Vercel production deploy is still blocked by account daily API deploy quota (`api-deployments-free-per-day`). Live production remains the previous READY deployment `dpl_EGaWZXC84K1MJAXSkdkBSjbnyFLm`; do not treat `npm run smoke:deploy-live` as proof that P10-04/P11-01/P11-02 are live until a new deployment succeeds.
  - Current compact checklist focus: P0-05 remains `LIVE` only because full provider-vault proof still includes a Kimi billing-cycle quota blocker. Cloudflare deploy auth, Worker deployment, Mimo vault rotation, Google/Gemma route proof, WeatherAPI proof, and Notion database permission proof are now working with redacted output. Keep broker-vault output redacted and secrets out of git.
