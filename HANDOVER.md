...
- 2026-06-08 compact P4 progress:
  ...
  - Verification for P4-01: first gate attempt correctly exposed a missing dev-server lifecycle (`ERR_CONNECTION_REFUSED`); the gate was fixed to own the dev server lifecycle. Latest `npm run smoke:production-gate` passed in 74s, including `smoke:final-nav` (8 passed), `smoke:mobile-layout`, `smoke:contact-sheet`, `smoke:broker-live`, `security:scan`, and `build`. Latest contact sheet: `/tmp/compact-contact-sheet-2026-06-08T07-48-02-202Z/mobile-contact-sheet.png`.
  - Next compact checklist focus: P4-02, an optional authenticated broker-vault verification workflow that must fail closed without local/session input and must redact provider output. Keep P0-05 `LIVE` until that proof exists.
