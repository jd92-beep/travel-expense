...
- Latest compact app-code proof commit: `b69887e` (`Compact Home information density pass`).
- Latest compact production deployment: Vercel deployment `dpl_BfhqqyhVNaUi6fSCfKEi9Dw9DhUa`, live URL `https://travel-expense-compact.vercel.app/`, HTTP 200, title `旅費 Compact`, asset hash `366bc6374213ab04`, HTML hash `4142c0335c4c6f17`, and alias/deployment content match for commit `b69887e`.
...
- 2026-06-09 compact Record declutter pass: History/Record no longer shows large `Itinerary Review Queue`, `Attachment Health`, or `Cleanup Coach` cards above the receipt list. Itinerary review and cleanup repair actions now appear only as a compact `Review only when needed` shortcut strip when active issues exist. Attachment/photo health moved into Settings `Compact Trip Doctor` as an `Attachments` item, while individual Record rows keep small `photo large`, `photo missing`, and `photo unsynced` markers. Verification passed: GitNexus impact on `History` and `Settings` was LOW; `npm run typecheck`; `node scripts/run-with-dev-server.mjs -- npm run smoke:history` (8 passed); `node scripts/run-with-dev-server.mjs -- npm run smoke:settings` (8 passed); `node scripts/run-with-dev-server.mjs -- npm run smoke:final-nav` (8 passed); `npm run smoke:production-gate` (83.7s); `npm run smoke:deploy-live` for deployment `dpl_2ZEBiwXihz3VtxsQMQwuEYhzWG93`.
...
- 2026-06-08 compact P4 progress:
  ...
  - P6-03 done: Compact History added cleanup suggestions for Pending OCR, Duplicate SourceID, Missing photo, and Missing payer. Current UI note: the old large `Cleanup Coach` panel is superseded by the 2026-06-09 Record declutter pass; cleanup actions now live in the compact `Review only when needed` shortcut strip when active issues exist. Each shortcut opens the first matching receipt or pending confirmation flow and does not add schema or API calls.
  ...
  - P7-04 done: Compact now has shared per-day readiness scoring through `buildDayReadinessScores()` in `app-compact/src/lib/travelDay.ts`. Dashboard renders a full `Day readiness` strip under the travel-day control panel, while Timeline renders the current day's readiness as an inline widget so the command/travel-day area stays below the existing mobile height guard.
  - P7-04 scoring inputs: itinerary coverage, active-trip route freshness, cached/local weather freshness and weather risk, stale booking references older than 30 days, completed-spot receipt gaps up to the actual time of day, and cleanup markers for pending OCR, duplicate SourceID, missing expected photos, large/unsynced local photos, sync conflict, and missing payer. It does not add storage fields and does not call external APIs or expose credentials.
  ...
  - P10-03 done: Dashboard now has an `Itinerary Receipt Match` / `每日紀錄對帳` card backed by `buildItineraryReceiptReconciliation()` in `app-compact/src/lib/travelDay.ts`. It compares current-trip receipts against itinerary days, flags no-receipt itinerary days, spot-level receipt gaps, unusually high receipt counts, and current-trip spending dates outside the itinerary. It links to Records, Stats, and Timeline for repair/review.
  ...
  - P11-02 done in local code: History has itinerary receipt review actions backed by existing `buildItineraryReceiptReconciliation()` logic. Current UI note: the old large `Itinerary Review Queue` card is superseded by the 2026-06-09 Record declutter pass; missing receipt days, high-count days, and outside-itinerary spending now appear as compact shortcut pills with the same one-tap date review filter and `All records` reset.
