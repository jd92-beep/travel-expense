...
- 2026-06-08 compact P4 progress:
  ...
  - P11-02 done in local code: History now has an `Itinerary Review Queue` card backed by existing `buildItineraryReceiptReconciliation()` logic. It lists missing receipt days, high-count days, and outside-itinerary spending, then applies a one-tap date review filter with an `All records` reset.
