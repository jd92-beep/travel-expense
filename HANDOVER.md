...
- Latest P0-05 preflight evidence: 2026-06-09 re-audit found no ignored `.broker-vault-session.local.json` in repo/app roots. `npm run smoke:broker-live` passed with `/health` 200 (`travel-expense-credential-broker`, version `2026.05.29`), CORS preflight 204 for `https://travel-expense-compact.vercel.app`, protected Notion/Kimi/Google/Mimo/WeatherAPI/credentials paths 401 `Session missing`, and no sensitive-looking response text. `npm run smoke:broker-vault:guard` also passed, proving the authenticated proof workflow fails closed without a local session. A normal `npm run smoke:broker-vault` run safely stopped with `status: blocked` / `missing local broker vault session`; provider vault calls were not executed. This does not prove provider vault contents; authenticated provider tests still require a safe ignored local broker session or Supabase token via `npm run smoke:broker-vault`.
